...
```
# 仕上げAIへ図表制作依頼
「一つのイベントを6要素へ分解する」というフレームワークを一枚で示す図を作成してほしい。テーマ、達成目標、報酬構造、進行状況の可視化、救済、終了状態という6要素を円環状または放射状に配置し、これらが一つのイベント仕様を構成する要素であることが視覚的にわかる構成にしてほしい。ファイル名は event-design-six-elements-framework.webp とする。
```
...
```
# 仕上げAIへ図表制作依頼
達成目標の三層構造を示す図を作成してほしい。参加目標（イベントの遊び方を理解させる）、完走目標（主報酬まで到達させる）、挑戦目標（余力のある層へ追加目標を渡す）をピラミッド状に積み上げ、下から上に向かって到達難度が上がることが視覚的にわかる構成にしてほしい。ファイル名は event-goal-three-tier-pyramid.webp とする。
```
...
```
# 仕上げAIへ図表制作依頼
開催期間を横軸としたタイムライン図を作成してほしい。序盤（参加費や編成負担を軽くする消耗品）、中盤（完走を予告する目立つ報酬）、主報酬（一般的なプレイ量で届く位置）、終盤（称号・外見・追加収集品などの挑戦報酬）という4つの区間を時間軸上に配置し、報酬の性質が開催期間の進行とともにどう変化するかを示す構成にしてほしい。ファイル名は event-reward-placement-timeline.webp とする。
```
...
```
# 仕上げAIへ図表制作依頼
FOMOの許容境界を示すスペクトラム図を作成してほしい。左端を「許容しやすい催促」（実際の終了日時を正確に表示する、残り時間と未交換品を知らせる等）、右端を「強制感が強くなる境界」（期限切れのたびに同じオファーが再出現する、休んだ日の進捗を完全に失わせる等）とし、両端の代表例を短く添えた連続的な軸として示す構成にしてほしい。ファイル名は event-fomo-spectrum.webp とする。
```
...
```
# 仕上げAIへ図表制作依頼
イベントの状態遷移図を作成してほしい。予告期間→開催期間→獲得終了→集計期間→交換猶予→確定・配布→アーカイブの7状態を矢印でつなぐフローチャートとし、各状態の下に一言でその状態の役割（例：獲得終了＝新規ポイントの獲得を止める）を添える構成にしてほしい。「開催中」「終了」の二状態だけでは足りないという本文の主張を裏付ける図であることを意識してほしい。ファイル名は event-lifecycle-state-transition.webp とする。
```
...
```
# 仕上げAIへ図表制作依頼
振り返りの基本ファネル図を作成してほしい。イベント対象になった→告知またはイベント画面を見た→最初のプレイを開始した→最初のプレイを完了した→主報酬へ到達した→挑戦目標へ進んだ→終了翌週もゲームを利用した、という7段階を漏斗状に配置し、各段階間で人数が減っていく（離脱が生じる）様子が視覚的にわかる構成にしてほしい。ファイル名は event-retrospective-funnel.webp とする。
```
