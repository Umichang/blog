...
```
# 仕上げAIへ画像引用依頼
EA公式プレスキット（ir.ea.comまたはea.com/newsのBattlefield 6関連発表）から、公式キーアートまたは発表用スクリーンショットを1点引用してほしい。「発売3日で700万本、シリーズ史上最大のローンチ」という本文の記述を補強する目的で使用する。出典をEA公式と明記し、無改変で掲載すること。適切な画像が見つからない場合はその旨を明記してスキップしてよい。
```
...
```
# 仕上げAIへ図表制作依頼
上記の交渉経緯を横軸タイムライン図にしてほしい。2025年9月26日（交渉報道）を起点とし、9月29日（正式合意）、12月22日（株主承認）、2026年3月（HSR待機期間満了・ローン販売開始）、6月（EU審査開始）、7月22日（EU競争法期限）、7月30日（EU外国補助金規制期限）、9月28日（CFIUS外部締切）までを時系列に並べる。現在地点（2026年7月）が分かるようにマーカーを入れる。
```
...
```
# 仕上げAIへ図表制作依頼
上表の出資比率（PIF約93.4%／Silver Lake 5.5%／Affinity Partners 1.1%）を円グラフで視覚化してほしい。PIFの区画が全体の9割超を占めることが直感的に伝わる構図にし、「実質的にサウジアラビア国家ファンドの傘下に入る」という直後の本文の主張を裏付ける図にする。
```
...
| 会社 | 保有比率（目安） | 備考 |
|------|---------|--------------|
| Nintendo | 数％台まで縮小傾向 | 2022年に約5%で取得後、2024年後半から段階的に売却を継続。2024年11月時点で7.54%→6.3%、同年12月に5.26%、直近では4%台まで低下している[[16](#ref-16)][[17](#ref-17)] |
| Nexon／Koei Tecmo／NCSoft／Square Enix Holdings | いずれも約10% | 2026年1月、PIFはこれらの株式を含む約120億ドル相当をSavvy Games Groupへ移管すると発表[[18](#ref-18)] |
| Capcom | PIF本体で約5%、別法人EGDC（Electronic Gaming Development Company）で約6.04% | PIFとEGDCはいずれもサウジアラビア系の投資主体だが別法人であり、2026年4月時点でEGDCが5%から6.04%へ持ち分を引き上げたことが関東財務局への大量保有報告書で判明している[[19](#ref-19)] |
...
```
# 仕上げAIへ図表制作依頼
上記2つのASCII図（「上場企業時代の決定構造」「LBO後の決定構造」）を統合し、正式なフローチャートとして再構成してほしい。左側に上場企業時代（株主→経営陣→各スタジオ、四半期利益への圧力を矢印ラベルで表示）、右側にLBO後（PIF約93.4%株主→経営陣（Andrew Wilson CEO続投）→各スタジオ、200億ドルのデット返済圧力を別経路の矢印で表示）を並べ、両者の違いが一目で対比できる構図にする。色調は本文の重い財務トピックに合わせ、過度に明るい配色は避ける。
```
...
```
# 仕上げAIへ図表制作依頼
EA（PIF主導のLBO）・Microsoft×Activision Blizzard・Embracer Groupの3事例を比較する図を作ってほしい。横軸に「取引総額」、縦軸に「買収側が負った借入金額（Microsoftは0ドル、Embracerは約15億ドル、EAは約200億ドル）」を取ったバブルチャートまたは棒グラフとし、EAの負債依存度の高さが視覚的に際立つ構成にする。第6章の本文・表で述べた数値と整合させること。
```
...
EAが非上場化することで、機関投資家はゲーミングエクスポージャーの再配分を迫られる。発表直後、Take-Two Interactiveの株価は約4.6%上昇したと報じられており、市場では同社がEAに代わる「NASDAQ最大のピュアプレイ（純粋な）大手ゲームパブリッシャー」というポジションを引き継いだという見方が出ている[[33](#ref-33)]。ただしTake-TwoはGrand Theft Auto VIの発売延期（2026年11月19日へ）を経験しており、ゲーム一作の遅延がいかに株価に影響するかを示す事例でもある[[34](#ref-34)][[35](#ref-35)]。
...
2020〜2023年の高金利環境でLBOは停滞していたが、2025年以降は複数の分析機関が「プライベートエクイティ業界が数兆ドル規模の未投資資金（ドライパウダー）を抱えている」と指摘しており、その規模の推計は１兆ドル台後半から3兆ドル台まで、集計方法によって幅がある。EA案件の成功が、類似のソフトウェア・エンターテインメント企業への大型LBOが連続する契機になりうるとの見方があり、この文脈でUbisoft・Square Enixなどが潜在的な次の対象として観測されている。
...
<a id="ref-16"></a>16. [Saudi Arabia's Sovereign Wealth Fund PIF Trims Nintendo Stake Again][16] - Bloombergによる2024年11月のNintendo株売却報道。

<a id="ref-17"></a>17. [The Public Investment Fund reduces its holdings in Japanese Nintendo from 5.26% to 4.19%][17] - PIFによるNintendo株の継続的な売却を報じた記事。

<a id="ref-18"></a>18. [Saudi Arabia's PIF to transfer $12bn in gaming shares to Savvy Games Group][18] - PocketGamer.bizによる、Nexon・Koei Tecmo等の株式のSavvy Games Groupへの移管報道。

<a id="ref-19"></a>19. [Saudi Arabia's Electronic Gaming Development Company Ups Its Capcom Stake][19] - thisweekinvideogamesによる、EGDC（PIFとは別法人）のCapcom出資比率引き上げの報道。
