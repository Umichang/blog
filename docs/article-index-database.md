...
> 本書は設計時点の記録である。分類の現行値は`data/article-categories.yml`が正であり、2026年8月の再分類（H4小見出しの分類への昇格、肥大した分類の分割、1件のみの分類の吸収）を経て35分類になっている。本書に登場する21カテゴリの記述とプロトタイプの`CATEGORY_SLUGS`は当時のものとして読むこと。
...
- カテゴリ台帳の初期値：8節のプロトタイプスクリプト内`CATEGORY_SLUGS`辞書をそのまま初期データとして使ってよい（本書執筆時点の21カテゴリぶんのslug案を含む。現行の35分類は`data/article-categories.yml`を参照）。slugの命名はCodex・うみちゃんの判断で変更して構わない
...
# 本書執筆時点（21カテゴリ）のindex.md見出し → 英語slugの初期マッピング（増減・改名自由）
CATEGORY_SLUGS = {
