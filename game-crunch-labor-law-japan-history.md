...
```
# 仕上げAIへ図表制作依頼
本セクション（2. なぜ「開発終盤」に集中するのか）のまとめとして、4つの構造要因が開発終盤に向けて重なり合う様子を示す循環図（フローチャート）を作成してください。
- 要素：①見積もりの甘さ（初期段階）→②仕様変更の連鎖（開発中期）→③バグ修正とQAの集中（開発後期）→④承認フローと外圧（マスターアップ直前）
- 時系列（左から右、または上から下）で並べ、各要素が次の要素の負荷を増大させる矢印でつなぐ
- 最終的に「クランチ」に帰結する構造を視覚的に示す
- 配色は本文の他の図表と統一感を持たせる
```
...
```
# 仕上げAIへ図表制作依頼
専門業務型裁量労働制における「ゲーム用ソフトウェアの創作業務」の対象・対象外を整理する簡易フローチャートまたは対比図を作成してください。
- 対象：ゲームの企画、シナリオ制作、プログラム創作（企画・設計段階に裁量があるプログラム創作を含む）
- 対象外：プログラムの設計または作成のみを行うプログラマー（指示通りに実装作業を行う場合）
- 「肩書きではなく実際の業務内容で判断される」という注意書きを図中に含める
```
...
```
# 仕上げAIへ図表制作依頼
上記の表「調査・出典 / 2018年 / 2022〜2024年」の内容を折れ線グラフまたは棒グラフとして可視化してください。
- 系列：Doda（ゲームクリエイター）2018年45.3時間→2024年25.6時間、ファミ通調査（ゲーム制作・開発職）2018年45.3時間→2023年24.4時間、CESA調査（繁忙期・週あたり）2024年52.8時間（通常期44.6時間も参考値として併記可）
- 単位の違い（月あたり／週あたり）が誤解を招かないよう軸ラベルを明確に分ける（できれば2軸グラフ、または月次と週次を別チャートに分割）
- 「2018年比で約半減」というトレンドが視覚的に伝わるようにする
```
...
```
# 仕上げAIへ画像引用依頼
CESA「ゲーム開発者の就業とキャリア形成2024」（https://www.cesa.or.jp/uploads/2025/info20250120-1.pdf）内の勤務形態の推移を示す図（図2-15-1、裁量労働制・フレックスタイム制度・通常の勤務時間制度の比率変化）を、上記の分析の裏付けとして引用してください。
- 引用要件（必要な範囲・本文が主／引用が従・出典明記・無改変）を満たす形でのみ使用すること
- 出典として「CESA『ゲーム開発者の就業とキャリア形成2024』より引用」を明記
- 要件を満たす形での掲載が困難な場合は、掲載を見送り、その旨を報告すること
```
...
```
# 仕上げAIへ画像引用依頼
厚生労働省リーフレット「裁量労働制の導入・継続には新たな手続きが必要です」（https://www.mhlw.go.jp/content/001080850.pdf）内の、2024年4月改正に関する手続きフロー図（本人同意の取得・記録保存等）を、上記の解説の裏付けとして引用してください。
- 官公庁資料のため著作権上の制約は比較的緩やかだが、出典明記（「厚生労働省リーフレットより引用」）は行うこと
- 引用要件（必要な範囲・本文が主／引用が従・無改変）を満たす形でのみ使用すること
- 要件を満たす形での掲載が困難な場合は、掲載を見送り、その旨を報告すること
```
...
```
# 仕上げAIへ図表制作依頼
2019年から2024年にかけての労働法制改正の流れをまとめた横型タイムライン図を作成してください。
- 2019年4月1日：時間外労働の上限規制（大企業、罰則付き）
- 2020年4月1日：中小企業への上限規制適用拡大
- 2023年4月1日：月60時間超の割増賃金率50%以上、中小企業への適用（猶予措置撤廃）
- 2024年4月1日：専門業務型裁量労働制の改正（本人同意の義務化、健康・福祉確保措置の強化）
- 各ポイントに簡潔な説明を添え、大企業／中小企業で適用時期が異なる点が分かるよう視覚的に区別する
```
...
新人ゲームプランナーの立場から実践可能な対策を整理する。

### 8-1. スコープ管理とMVP設計

開発の途中で仕様変更が発生した際、「スケジュールを守るか、スコープを削るか、品質を下げるか」の三択を明示的に議題に上げることが重要だ。スクラム開発の知見を援用するなら、「リリースはできるだけ小さな単位で、頻繁に行う」ことで、終盤への仕様積み残しを防ぐことができる。[[40](#ref-40)]
