...
```
# 仕上げAIへ図表制作依頼
対話をメカニクスとして成立させる条件を、左から「情報」「発話」「他者の解釈」「ゲーム状態の変化」へ流れる横長の4段フローで表現する。各段に、非対称な情報、公開・非公開の会話、信頼・疑念・投票、役割・選択肢・勝敗条件の変化を配置する。ボタン操作との違いとして、発言内容を設計者が全列挙せず、制約と結果の接続を設計することを短い注記で示す。日本語の文字量は少なめにする。
```
...
```
# 仕上げAIへ図表制作依頼
TRPG、人狼、マーダーミステリーを横一列に置き、下段に「自由な行動宣言」「非対称な情報」「秘密と個別目的」を積み上げ、右端で「対話が勝敗・進行・物語の状態を変える」という共通項へ収束する系譜図を制作する。各ジャンルを単純な直系の進化として描かず、「設計要素の蓄積」と注記する。年表ではなく、メカニクスの重なりが主役になる構成にする。
```
...
```
# 仕上げAIへ図表制作依頼
Among Usの進行構造を、「行動フェーズ」と「討議フェーズ」を左右または上下に分けた2フェーズ図として制作する。行動フェーズ側には移動・タスク・妨害・断片的な目撃情報を、討議フェーズ側には死体報告・緊急会議・議論・投票を配置し、両フェーズが交互に切り替わることを矢印で示す。会話が常時発生するのではなく、フェーズ単位で発生条件が変わる点が伝わる構成にする。日本語の文字量は少なめにする。
```

```
# 仕上げAIへ画像引用依頼
Innersloth公式プレスキット（https://www.innersloth.com/press-kit/）で配布されているAmong Usのキーアート画像を1点引用する。複数のクルーメイトが並んだプロモーション用アートワーク（例：15player_promo_noText.png）が望ましい。出典として「Innersloth」を明記し、無改変で掲載する。掲載位置はこの段落の直後とする。
```
...
```
# 仕上げAIへ図表制作依頼
プランナー向けの対話メカニクス検証ループを、上段に「目的を定義」、左下に「情報と役割を配る」、中央下に「会話の境界を設定」、右下に「行動と結果へ接続」、下段から「プレイテストで判断の変化を観察」して上段へ戻る循環図として制作する。各ノードの文言は短くし、UX、ストーリーテリング、システム設計が同じループに入ることを示す。装飾より実務の読みやすさを優先する。
```
