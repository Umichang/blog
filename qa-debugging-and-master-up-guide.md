---
description: "QA、デバッグ、マスターアップを、限られた時間で品質と発売判断を両立させる工程として整理し、優先順位の付け方を解説する。"
---
...
つまり終盤の仕事は、バグをゼロにすることではない。 **限られた時間で重大なリスクを減らし、残るリスクを説明できる状態にして、出荷の可否を決めること** である。ISTQB（International Software Testing Qualifications Board、ソフトウェアテストの国際資格認定機関）のテスト原則でも、自明な小規模ケースを除けば網羅的なテストは不可能であり、テスト技法、優先順位付け、リスクベースドテストによって対象を絞るべきだとされている。[[1](#ref-1)]
...
JSTQB（Japan Software Testing Qualifications Board、ISTQBの日本における資格認定委員会）のゲームテスティングシラバスは、ゲームデザイナーの責任にゲームメカニクスや文書の設計を、テスト担当者の責任に戦略、テストデータ、仕様レビュー、各種テスト、欠陥分析を挙げている。また、テスト計画は完成間際ではなく、コンセプトやプリプロダクションの段階から作るものとしている。[[2](#ref-2)]
