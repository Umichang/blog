...
```
# 仕上げAIへ図表制作依頼
第4章の内容に対応する概念図を作成してください。「投げる」→「くっつく」→「運ぶ」→「増える」という4つの行為が、宮本氏の「投げつけた後どうなるの？」という問いかけをきっかけに、一段ずつ連鎖的に発見されていった過程を示してください。最初から4機能が並んでいたのではなく、前の行為の結果を見て次の行為が生まれたという順序性が伝わる図にしてください。数値要素は不要です。
```
...
```
# 仕上げAIへ図表制作依頼
第5章の内容に対応する概念図を作成してください。AI試作・キャラクターデザイン・コアループという断片的な要素が、宮本氏の1枚のゲームフロー図へ統合される様子を示してください。あわせて「50匹運んだらクリア」という恣意的なゴールの書き方と、「重さに応じて必要な仲間の数が変わる」という因果ベースのゴールの書き方を対比する要素も含めてください。数値要素は不要です。
```
...
```
# 仕上げAIへ図表制作依頼
『ピクミン』本編シリーズの発売年表チャート（横棒または年表形式）を作成してください。
データと出典は次のとおりです。
- 2001年10月26日 ピクミン（ニンテンドーゲームキューブ）／2004年4月29日 ピクミン2（ニンテンドーゲームキューブ）／2013年7月13日 ピクミン3（Wii U）／2023年7月21日 ピクミン4（Nintendo Switch）：出典は任天堂公式「ピクミンのヒストリー」（記事References [1]）
月日まで確定しているのは上記4作すべてです。移植版・スピンオフ（Hey!ピクミン、ピクミン3 デラックス、ピクミン ブルーム等）は対象外とし、本編4作のみを扱ってください。
```

```
# 仕上げAIへ画像引用依頼
初代『ピクミン』（2001年、任天堂、ニンテンドーゲームキューブ）のプレイ画面で、生き物の群れを率いて敵と戦う、または獲物を運ぶ様子が確認できる画像を、著作権法上の引用要件（必要な範囲・主従関係・出典明記・無改変）を満たす形で使えるか検討してください。任天堂の公式サイトや公式資料に掲載されている画像を優先して探し、要件を満たす候補が見つからない場合は掲載を見送ってください。
```
