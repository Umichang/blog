...
```
# 仕上げAIへ画像引用依頼
クリントン・キース氏の著書『Agile Game Development with Scrum』の書影を引用候補として検討してください。本文でこの書籍を具体的に論じているため、書影を主たる文章に対して従たる形で掲載できるか、著作権法上の引用要件（必要な範囲・主従関係・出典明記・無改変）を満たすかご確認のうえ、満たす場合のみ採用し、出典を明記してください。要件を満たす掲載方法が見つからない場合はスキップしてください。
```
...
└── スクラム
    └── スプリント
        ...
        ├── スプリントレビュー
        └── スプリントレトロスペクティブ（振り返り）
            └── KPT、スターフィッシュ、4Ls などの進め方の型
```

```
# 仕上げAIへ図表制作依頼
本文中のツリー表記（アジャイル開発→スクラム→スプリント→スプリントプランニング／デイリースクラム／スプリントレビュー／スプリントレトロスペクティブ→KPT・スターフィッシュ・4Lsなどの進め方の型）を、正式な階層図として作成してください。「KPTをやっているからスクラムをやっているわけではない」という本文の主張が一目で伝わるよう、KPTがスプリントレトロスペクティブという1イベントの中の1手法に過ぎないという入れ子構造を強調してください。
```
...
```
# 仕上げAIへ図表制作依頼
「イベントをこなしても、意識が揃わない理由」の章で挙げた4つの原因（手続きだけが残り背景の考え方が共有されていない／スクラムマスターが管理職の代役になる／プロダクトオーナーの関与が弱い／ゲーム開発の探索性と固定的な儀式が衝突する）を整理する図を作成してください。4つを並列に示し、それぞれに本文中の具体的な兆候（付箋の枚数ではなくTryの追跡有無、進捗報告会化したデイリースクラム、名前だけの参加、探索的な工程との摩擦など）を短い言葉で添えてください。
```
...
```
# 仕上げAIへ図表制作依頼
CEDECにおけるゲーム開発とスクラムの実践報告の年表を作成してください。CEDEC2010（ゲームリパブリック、田中宏幸氏、スクラム基礎と導入事例）、CEDEC2012（同氏、スクラム＋XP＋アジャイルUX）、CEDEC2014（同氏、Hansoftによる運用事例）、CEDEC2015（同氏、チームを支える原動力）、CEDEC2019（ディライトワークス、導入の壁と課題）、CEDEC2020（同氏、アジャイル開発10年間の軌跡）、CEDEC2021（アカツキ、LeSS導入）、CEDEC2024（スクウェア・エニックス、スケールしないスクラムの事例）を時系列に並べ、単発の流行紹介ではなく継続的な試行錯誤の共有であることが伝わるようにしてください。
```
...
```
# 仕上げAIへ図表制作依頼
新人プランナーがKPTセッションでできる5つの行動をチェックリスト形式の図で作成してください。（1）Problemを人ではなく行動と仕組みで語る、（2）Keepが機能した理由を言葉にする、（3）自分が出したTryを自分で追跡する、（4）ファシリテーターでなくても発言の安全性を支える、（5）形骸化に気づいたら観察可能な事実から相談する、の5点を、読者が明日から実践できる行動として端的に示してください。
```
