...
旧仕様では、1回募集するごとに「呼び出しポイント」を1点得た。200点に達すれば、ピックアップ対象の生徒と交換できる。募集終了時に残ったポイントは持ち越されず、キーストーンのカケラへ1対1で自動変換された。[[1](#ref-1)]
...
旧仕様の200点は、運の悪さを吸収する上限であると同時に、青輝石の予算を組む単位でもあった。「この2人を確保するには最悪400回」「片方が途中で来れば200回で済む」という見通しは、確率を細かく計算しないプレイヤーにも共有できる。
...
しかし旧仕様の200点は、単なる確率救済ではなかった。複数の生徒、所持青輝石、復刻予定を前提に「この回数なら確保できる」と決めるための契約だった。新仕様はその契約を、長期の平均を良くする代わりに、途中の当たりで進捗が消える別の契約へ置き換えた。
