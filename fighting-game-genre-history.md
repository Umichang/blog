...
```
# 仕上げAIへ画像引用依頼
『空手道』（データイースト、1984年）のアーケード版プレイ画面を1点、著作権法上の「引用」要件（報道・批評・研究等の目的上必要な範囲であること、本文が主・引用が従の関係にあること、出典明記、無改変）を満たす形で使用できないか確認してください。
候補：アーケードアーカイブス公式サイト（https://www.arcadearchives.com/title/aca-012/）掲載の公式スクリーンショット。
用途：本段落（『空手道』は競技を操作へ落とし込んだ）の挿絵として、2本レバーの操作感や当時のグラフィックが伝わるもの。
要件を満たす画像が見つからない場合はスキップしてください。
```
...
```
# 仕上げAIへ画像引用依頼
『ストリートファイターII』（カプコン、1991年）の対戦シーン、または当時のゲームセンターにおける背中合わせの対戦台の様子を1点、著作権法上の「引用」要件（報道・批評・研究等の目的上必要な範囲であること、本文が主・引用が従の関係にあること、出典明記、無改変）を満たす形で使用できないか確認してください。
候補：Capcom Town公式ページ（https://captown.capcom.com/ja/classic_games/22）掲載の画像。
用途：本セクション（『ストリートファイターII』が実際に確立したもの）の挿絵として、多彩なキャラクター選択や対戦台の雰囲気が伝わるもの。
要件を満たす画像が見つからない場合はスキップしてください。
```
...
# 仕上げAIへ図表制作依頼
フレームデータの概念を図解してください。
・横軸を時間（フレーム経過）としたタイムライン形式（帯グラフのイメージ）
・技の入力から攻撃判定が出るまでの「発生」
・攻撃判定が出続ける「持続」
・攻撃判定が消えてから再び動けるようになるまでの「硬直」
・技がヒット、またはガードされた場合に、どちらが先に動けるかを示す「有利」「不利」の概念
本文では具体的な数値を挙げていないため、図中にも架空の数値は入れず、帯の長さの相対関係のみで構成を示す概念図としてください。攻撃側・防御側で帯を上下に並べる構成が望ましいです。
```
...
```
# 仕上げAIへ図表制作依頼
攻撃判定・やられ判定（喰らい判定）・押し合い判定の3種類を図解してください。
・向き合う2体のキャラクターのシルエットを横から見た概念図
・攻撃を出している側のキャラクターに、技の届く範囲として「攻撃判定」を色分けした図形で重ねて表示
・両キャラクターの身体に「やられ判定（喰らい判定）」を別の色で重ねて表示
・両キャラクターの胴体部分に「押し合い判定」を示す枠を表示
・キャラクターの見た目の輪郭と、判定図形の範囲が必ずしも一致しないことが一目で伝わるよう、絵と判定図形を意図的にややずらして重ねる表現にしてください
・3種類の判定を色分けし、凡例を添えてください
```
...
```
# 仕上げAIへ画像引用依頼
『バーチャファイター』（セガ、1993年）のゲーム画面を1点、著作権法上の「引用」要件（報道・批評・研究等の目的上必要な範囲であること、本文が主・引用が従の関係にあること、出典明記、無改変）を満たす形で使用できないか確認してください。
候補：セガ公式ヒストリーページ（https://www.sega.jp/history/arcade/product/9122/）掲載の画像。
用途：本段落の挿絵として、業界初のフルポリゴン3D対戦格闘という特徴が伝わるもの。
要件を満たす画像が見つからない場合はスキップしてください。
```
...
```
# 仕上げAIへ画像引用依頼
Evo 2004「Evo Moment 37」（梅原大吾 対 Justin Wong、『ストリートファイターIII 3rd STRIKE』）の一場面を示す静止画を1点、著作権法上の「引用」要件（報道・批評・研究等の目的上必要な範囲であること、本文が主・引用が従の関係にあること、出典明記、無改変）を満たす形で使用できないか確認してください。
候補：Guinness World Records公式ページ（https://www.guinnessworldrecords.com/world-records/most-viewed-competitive-videogame-match）掲載の画像、または権利者の許諾のもとで公開されている報道用画像。
用途：本セクション（「背水の逆転劇」は、なぜ伝説になったのか）の挿絵として使用。
適切な引用要件を満たす画像が見つからない場合はスキップしてください。
```
...
**ロールバック方式** は、届いていない相手入力を予測して先に進め、後から実入力が届いて予測と違えば、過去の状態へ戻して再計算する。GGPOはこの方式を対戦ゲーム向けSDKとして普及させた。公式説明では、同じ入力なら同じ結果になる決定論的なシミュレーション、状態の保存と復元、描画せず高速に再実行する仕組みが必要とされる。[[16](#ref-16)]

```
# 仕上げAIへ図表制作依頼
従来のディレイ方式とロールバック方式の違いを、比較図またはフローチャートで示してください。
・上段に「ディレイ方式」：相手の入力が届くまで自分の入力反映を遅らせて双方の状態を合わせる、という直列的な流れを表現
・下段に「ロールバック方式」：以下4ステップを矢印でつないだ流れとして表現
　①相手の入力を予測して先に進める
　②実際の入力が届く
　③予測が外れていた場合、過去の状態へ巻き戻す
　④正しい入力で再計算する
・専門用語（決定論的シミュレーション、状態保存など）は図中では最小限にし、本文の説明を補う位置づけにしてください
・上下2方式を対比しやすいレイアウトにしてください
```
...
公正さには、通信同期だけでなくマッチメイキングと不正対策も要る。実力差と回線品質を同時に評価し、改造クライアントや入力自動化をどこで検知するかを決める。すべてをサーバーで計算すれば費用が増え、端末を広く信頼すれば改ざん余地が増える。大会賞金、ランクの価値、対応機種に応じて、リプレイ監査、通報、実行環境の検査などを組み合わせる必要がある。通信方式の詳細は[家庭用オンラインマルチプレイの遅延対策](console-online-multiplayer-latency-techniques.md)、対戦相手の選び方は[レーティングとマッチメイキングの設計](competitive-game-matchmaking-rating-explained.md)、不正対策の層は[オンラインゲームのアンチチート技術史](online-game-anti-cheat-technology-history.md)で掘り下げているが、本稿の要点は「対戦の公正さはバトル部分だけでは閉じない」ということだ。

## e-sports化と現在――入口を広げても、読み合いは残せるか
...
なし。起源・初出に関して資料間で揺れやすい箇所は、メーカーが明示した範囲に限定するか、初出断定を避けた。
