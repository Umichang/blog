...
```
# 仕上げAIへ図表制作依頼
P2Eゲームの収益構造を示す概念図を作成してください。中央に「運営」を置き、そこへ入ってくる収益として「一次販売収入(NFT初回販売・ゲーム内通貨・ガチャ・参加費)」「二次流通手数料(ユーザー間NFT売買)」「トークン需要(市場価格・保有動機)」の3種類を矢印で接続してください。同時に、運営から出ていくコストとして「通常のゲーム運用費」と「Web3固有の運用費(外部ウォレット連携・NFT発行・マーケットプレイス連携・監査・会計税務・価格変動対応)」の2種類を対比的に配置し、収益の種類によって発生タイミングや現金化のしやすさが異なることが視覚的に伝わる構成にしてください。
```
...
```
# 仕上げAIへ画像引用依頼
『TOKYO BEAST TRIALS』の3Dオートバトル、またはBEASTのキャラクタービジュアルが分かる公式配布の画像を探してください。公式サイト（tokyo-beast.com）や公式note、プレスリリース内で公開されているスクリーンショット・キービジュアルが候補になります。著作権法上の引用要件（必要な範囲・主従関係・出典明記・無改変）を満たすものに限り使用し、満たすものが見つからない場合は掲載を見送ってください。
```
...
```
# 仕上げAIへ図表制作依頼
2025年3月3日の事前登録開始から2025年9月30日の払戻し受付終了までを横軸とした年表形式のタイムライン図を作成してください。「立ち上がり」側（事前登録開始・事前登録100万人突破・グローバルリリースと初日の反響）と「終わり」側（サービス終了告知・段階的な機能停止・払戻し受付終了）を視覚的に対比できるよう、色分けや配置を工夫し、グローバルリリース（6月9日）からサービス終了告知（7月22日）までが43日、サービス終了（8月24日）までが76日という短さが一目で伝わる構成にしてください。
```
...
### AMAの意見を受け、熟練度基準へ修正した

初回告知後、運営は7月24日にDiscordでAMAを実施し、7月25日にBEAST NFTの補填基準を「BEASTランク別」から「熟練度」に応じた設定へ変更した。熟練度が高いほど補填額を増やす方式である。[[9](#ref-9)]
