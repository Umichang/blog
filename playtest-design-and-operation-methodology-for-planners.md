...
```
# 仕上げAIへ図表制作依頼
開発フェーズを横軸、プレイテストで確かめる対象を縦軸にした2次元の整理図を制作すること。
左から「プロトタイプ」「α版」「β版」「ライブ運用後」を配置し、上から「理解・仮説」「コア体験」「完成度・環境差」「変更の影響」を対応させる。
各セルには短い語句だけを置き、プロトタイプほど定性・モデレート寄り、β版以降ほど定量・非モデレート寄りになる傾向を矢印で示す。ただし、どのフェーズでも両方を組み合わせられることが分かる構成にする。
```
...
**外部リクルーティング会社** は、地域、年齢、経験、利用環境などの条件が細かい場合や、短い期間で必要人数を集めたい場合に向く。その代わり、募集条件を曖昧に渡すと、会社側の一般的な「ゲーマー」定義でスクリーナーが作られる。GOV.UKは、募集会社には調査日、人数、対象条件、報酬などを明記したリクルート仕様書を渡し、返ってきたスクリーナーを発注側が確認するよう勧めている。[[4](#ref-4)]
...
定性プレイテストでは、少人数で具体的な失敗パターンを見つけ、修正後に別の参加者で再確認する反復が基本である。GOV.UKは、インタビューやユーザビリティテストの1ラウンドを通常4〜8人程度とし、必要なら1回に詰め込まず複数ラウンドに分ける考え方を示している。[[2](#ref-2)] NN/gも、定性的なユーザビリティテストでは5〜8人を目安としている。[[8](#ref-8)]
...
**think-aloud法** は、タスク中に考えていることや行っていることを声に出してもらう方法である。GOV.UKは、操作中の理解や迷いを知る手段として説明する一方、参加者が「正しい答え」を言おうとしたり、発話そのものが操作に影響したりする弱点も示している。[[9](#ref-9)] したがって、発話をプレイヤーの本心そのものとみなさず、操作・時間・失敗と照合する必要がある。
...
テレメトリーは、ゲーム内で起きたイベントを記録し、後から集計・比較するためのデータである。Microsoft PlayFabの公式ドキュメントでは、イベントを状態の変化として扱い、プレイヤー、時刻、関連データと結びつけて分析できると説明されている。[[10](#ref-10)]
...
PlayFabは、レベル完了時間、ログイン、レベルアップ、死亡などをイベントやメトリクスの例として挙げている。[[10](#ref-10)] 空間データを地図に重ねるヒートマップは、死亡地点や滞在地点の集中を発見するのに役立つが、なぜそこに集中したかまでは示さない。原因の候補を定性観察で確かめるべきである。

ライブ運用後は、計測したイベントの定義と品質も確認する必要がある。Unity Analyticsの公式ドキュメントでは、イベントマネージャーで標準イベントやカスタムイベントを管理し、受信した有効・無効イベントの状態を確認できるとされている。[[11](#ref-11)] データがあることと、問いに答えられる品質であることは別である。
...
良いタスクは、参加者に達成したい目的を与える。GOV.UKは、タスクを現実的で、十分に課題があり、答えや操作方法を直接示さない形にするよう勧めている。[[12](#ref-12)]
...
モデレート型では、参加者を評価するのではなく、ゲームの設計を調べていることを最初に伝える。GOV.UKも、参加者に「テストされているのはあなたではない」と安心させ、担当者は主に見て聞くよう勧めている。[[12](#ref-12)]
...
ライブ運用後のテストでは、プレイヤーアカウントとテストアカウントを分ける。課金、ランキング、フレンド通知、個人情報、サポート履歴に影響する機能を扱う場合は、データを本番環境へ流さない設計を優先する。PlayFabのような分析基盤でも、イベントの収集・保存・分析・削除の担当者と権限を決めておく必要がある。[[13](#ref-13)]

## 8. 分析は「感想集」ではなく、意思決定の材料に変換する

セッション終了後に全員が感想を言い合うだけでは、声の大きい人の解釈が残る。GOV.UKの分析指針は、調査の観察者を分析に参加させ、できるだけ早く、観察事実を整理・解釈し、次の行動を決める流れを示している。[[14](#ref-14)]
...
ただし、テレメトリーだけで因果関係を断定してはいけない。Microsoft PlayFabのPlayStreamは、プレイヤーやゲームからのリアルタイムイベントを処理し、行動変化やゲーム内容の調整に利用できる基盤として説明されている。[[13](#ref-13)] 便利な一方、同じ時期にイベント、価格、広告、サーバー状態が変わっていれば、指標の変化がどの施策によるものか分からない。比較条件、期間、セグメント、変更履歴を一緒に管理する必要がある。
...
担当者が「この機能は便利ですよね」と説明したり、詰まった直後に正しい操作を教えたりすると、問題が消える。質問は中立に、介入は記録付きで行う。NN/gも、参加者への影響を避けるため、テスト中はできるだけ黙って観察し、開放的で中立な質問をするよう勧めている。[[15](#ref-15)]
...
<a id="ref-9"></a>9. [Think aloud study: qualitative studies - GOV.UK][9] - think-aloud法の目的、利点、限界、実施方法に関する公式ガイダンス。

<a id="ref-10"></a>10. [Metrics and terminology - PlayFab][10] - ゲームイベント、メトリクス、セグメント、リテンションなどを説明するMicrosoft公式ドキュメント。

<a id="ref-11"></a>11. [Event Manager - Unity Analytics][11] - 標準イベント、カスタムイベント、有効・無効イベントを管理するUnity公式ドキュメント。

<a id="ref-12"></a>12. [Using moderated usability testing - Service Manual - GOV.UK][12] - モデレート型テストの目的、タスク、進行、観察に関する公式指針。

<a id="ref-13"></a>13. [PlayStream Overview - PlayFab][13] - プレイヤー行動イベントを収集・処理し、分析やライブ運用に利用するPlayFab公式ドキュメント。

<a id="ref-14"></a>14. [Analyse a research session - Service Manual - GOV.UK][14] - 観察データの整理、分析、意思決定、共有に関する公式指針。

<a id="ref-15"></a>15. [Why You Only Need to Test with 5 Users - Nielsen Norman Group][15] - 少人数の定性テストを反復する考え方と、参加者への影響を避ける原則を解説する記事。
...
[9]: https://www.gov.uk/guidance/think-aloud-study-qualitative-studies
[10]: https://learn.microsoft.com/en-us/xbox/playfab/data-analytics/learn-data/reports/metrics-and-terminology
[11]: https://docs.unity.com/en-us/analytics/events/event-manager
[12]: https://www.gov.uk/service-manual/user-research/using-moderated-usability-testing
[13]: https://learn.microsoft.com/en-us/xbox/playfab/data-analytics/ingest-data/playstream-overview
[14]: https://www.gov.uk/service-manual/user-research/analyse-a-research-session
[15]: https://www.nngroup.com/articles/why-you-only-need-to-test-with-5-users/
