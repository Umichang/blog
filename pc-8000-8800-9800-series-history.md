...
本稿ではPC-8001を出発点に、PC-8801がホビーとゲームの市場を育て、PC-9801がビジネス標準として巨大なソフト資産を抱え、最後にWindowsとDOS/Vの波で相対化されていく流れを整理する。なお、PC-98上の同人ソフト文化、Game Freakの創業前史、家庭用ゲーム機との音源比較は既存記事で扱っているため、本稿では深入りしない。
...
```
# 仕上げAIへ画像引用依頼
PC-8001本体の写真を引用候補として探してください。情報処理学会コンピュータ博物館のPC-8001解説ページ（本文ref1）に本体写真が掲載されています。引用にあたっては、必要な範囲にトリミングすること、本文が主・引用画像が従という関係を保つこと、出典（情報処理学会コンピュータ博物館）を明記すること、無改変とすることを満たしてください。
```
...
```
# 仕上げAIへ画像引用依頼
PC-8801本体の写真を引用候補として探してください。情報処理学会コンピュータ博物館の展示リストにPC-8801単体の解説ページがあるか確認し、あればその本体写真を、なければAKIBA PC Hotline!などの専門メディアに掲載された実機写真を候補としてください。引用要件（必要な範囲へのトリミング、本文が主・引用画像が従という関係、出典明記、無改変）を満たす適切な画像が見つからない場合は、その旨を明記した上でこの画像引用をスキップしてください。
```
...
```
# 仕上げAIへ画像引用依頼
PC-9801（初代機）本体の写真を引用候補として探してください。情報処理学会コンピュータ博物館のPC-9801解説ページ（本文ref2）に本体写真が掲載されています。引用にあたっては、必要な範囲にトリミングすること、本文が主・引用画像が従という関係を保つこと、出典（情報処理学会コンピュータ博物館）を明記すること、無改変とすることを満たしてください。
```
...
```
# 仕上げAIへ画像引用依頼
PC-286 MODEL 0（セイコーエプソンのNEC PC-98互換機）本体の写真を引用候補として探してください。AKIBA PC Hotline!の記事「ここから始まるNECとEPSONとの新たなる戦い『EPSON PC-286MODEL 0』」（本文ref6）に本体写真が複数枚掲載されています。引用にあたっては、必要な範囲にトリミングすること、本文が主・引用画像が従という関係を保つこと、出典（AKIBA PC Hotline!、Impress Corporation）を明記すること、無改変とすることを満たしてください。
```
...
```
# 仕上げAIへ図表制作依頼
1990年前後の日本PC市場における勢力図を作成してください。中心にNEC（PC-9801、市場シェア50%超）を大きく配置し、その周囲に以下の陣営を独立したノードとして配置してください：セイコーエプソン（NEC PC-98互換機）、Apple Japan（Macintosh）、富士通（FMR、FM TOWNSの2系統を明示）、東芝（独自ノートPC）、IBM Japan（PS/55、AX陣営の提唱者）、AX仕様陣営（三菱電機・シャープ・日立・沖電気・ソニー・三洋・キヤノンをまとめて1グループとして表記）。各陣営間には矢印や線を引かず、独立した互換性のない勢力として並列に配置することで「乱立」状態を視覚的に表現してください。図のタイトルは「1990年前後の日本PC市場（互換性の乏しい乱立状態）」としてください。
```
...
コンパイルのようなソフトハウスは、PC・MSX・家庭用機を横断しながら、シューティング、パズル、ディスクマガジン的な編集文化を育てた。『ザナック』のような作品は、後年「AI」や自動難易度調整の文脈でも語られるが、ここで注目すべきは技術名そのものではなく、PCショップ、雑誌、イベント、移植を横断するソフトハウスの機動力である。D4エンタープライズのEGGコンソール版『ZANAC』ページも、同作をMSX向け作品として現行配信しており、当時のPC系ソフトが保存・再流通の対象になっていることを示している。[[9](#ref-9)]
...
<a id="ref-9"></a>9. [ZANAC（MSX）｜EGGコンソール][9] - コンパイル作品の現行復刻・再流通例として参照した。
...
[9]: https://www.amusement-center.com/project/egg/console/detail.php?date=20250225&id=ecsw0052&lang=en
