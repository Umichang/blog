...
```
# 仕上げAIへ画像引用依頼
『ドラゴンボール ファイターズ』の公式キービジュアルまたはパッケージアートを引用候補として探してほしい。記事冒頭に置き、読者が題材となる作品を即座に把握できるようにする。著作権法上の引用要件(必要な範囲・主従関係・出典明記・無改変)を満たすことを確認し、出典を明記すること。
```
...
```
# 仕上げAIへ画像引用依頼
セルルック表現(アニメと見紛う3Dモデルのリアルタイム描画)が分かる公式スクリーンショットを引用候補として探してほしい。技術転用というこの章の核心を視覚的に裏付ける図として使う。著作権法上の引用要件(必要な範囲・主従関係・出典明記・無改変)を満たすことを確認し、出典を明記すること。
```
...
```
# 仕上げAIへ画像引用依頼
3対3のチーム編成画面、またはキャラクター交代・アシストが分かる公式スクリーンショットを引用候補として探してほしい。この章で解説する3対3の戦略性を視覚的に裏付ける図として使う。著作権法上の引用要件(必要な範囲・主従関係・出典明記・無改変)を満たすことを確認し、出典を明記すること。
```
...
```
# 仕上げAIへ図表制作依頼
横長の日本語インフォグラフィック。白地、黒とオレンジを基調に、ゲーム画面やキャラクター画像は使わない。4機種のMetacriticスコアを横並びで示す簡潔な図。PlayStation 4「87点(78件)」、Xbox One「86点」、PC「85点」、Nintendo Switch「87点」。見出しは「『ドラゴンボール ファイターズ』機種別Metacriticスコア」。文字化けを避けるため、文字数をこれ以上増やさない。
```
...
```
# 仕上げAIへ図表制作依頼
横長の日本語インフォグラフィック。白地、黒とオレンジを基調に、ゲーム画面やキャラクター画像は使わない。中央に右向きの一本のタイムラインを置き、5つの節目を大きく読みやすく示す。2018年2月「200万本」、2020年5月「500万本」、2021年1月「650万本」、2021年11月「800万本」、2023年5月「1000万本」。見出しは「『ドラゴンボール ファイターズ』累計出荷本数の推移」。小さな注記として「ダウンロード版を含む／各時点の公式発表」を入れる。棒グラフではなく、節目の円と数値を結んだ時系列図にする。文字化けを避けるため、文字数をこれ以上増やさない。
```
