...
```
# 仕上げAIへ画像引用依頼
『hololive Dreams』の実際のゲーム画面を1枚引用したい。候補は、無人島の「ドリーム・パーク」を運営する箱庭パート、またはリズムゲームのプレイ画面。電撃オンラインの先行レビュー記事（https://dengekionline.com/article/202607/79730）またはカバー株式会社の公式発表ページ（https://cover-corp.com/news/detail/20260723-01）に掲載されているスクリーンショットから、引用の要件（必要な範囲にとどめる、本文が主で画像が従、出典を明記する、画像に手を加えない）を満たす形で選定してください。著作権表記は「© COVER / © QualiArts, Inc.」、出典は掲載元メディア名またはカバー株式会社公式サイトを明記してください。要件を満たす画像が見つからない場合はスキップしてください。
```
...
```
# 仕上げAIへ図表制作依頼
タイトルは「『ホロドリ』の二層の参加経路」。数値グラフではなく、日本語の概念図として作成する。左に「カバー／QualiArts：公式楽曲・タレント・パーク・運営」、中央上に「『hololive Dreams』：公式のゲーム体験」、中央下に「クリエイト譜面：作る／共有する／遊ぶ」、右に「holo Indie：個人クリエイターの二次創作ゲーム」を配置する。矢印は、公式素材がホロドリとholo Indieへ向かうこと、ファンの創作がクリエイト譜面とholo Indieからほかのファンへ広がることを示す。下部に「共通する設計思想：ファンとの共創」と短く置く。固有名詞以外は短い日本語にし、文字化けを避ける。
```
...
```
# 仕上げAIへ画像引用依頼
holo Indie第一弾タイトル『ホロパレード』のゲーム画面またはキービジュアルを1枚引用したい。カバー株式会社の公式発表ページ（https://cover-corp.com/news/detail/20231115-02）に掲載されている画像、またはSteamストアページ（https://store.steampowered.com/app/2647230/HoloParade/）のスクリーンショットから、引用の要件（必要な範囲にとどめる、本文が主で画像が従、出典を明記する、画像に手を加えない）を満たす形で選定してください。出典は制作者「ろぼくろ」氏およびカバー株式会社であることを明記してください。要件を満たす画像が見つからない場合はスキップしてください。
```
...
```
# 仕上げAIへ図表制作依頼
タイトルは「活動終了後も残る価値、残らない価値」。数値グラフではなく、日本語の概念図として作成する。三つの箱を横に並べる。左に「記録として残る価値：楽曲・ライブ映像・会話・衣装・関係性」、中央に「現在の活動と結ぶ価値：新曲・配信連動・季節イベント・本人参加の更新」、右に「コミュニティに移せる価値：譜面・スコア・パークの見せ方・二次創作」を配置する。中央の箱にのみ「タレントの活動終了で失われやすい」という短い注記を添える。固有名詞以外は短い日本語にし、文字化けを避ける。
```
