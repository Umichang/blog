...
```mermaid
timeline
    title 家庭用ゲーム機サウンド進化の要点
    1983 : ファミコン
         : Ricoh 2A03 APU
         : 2 pulse + triangle + noise + DMC
    1986 : ファミコン ディスクシステム
         : 追加音源の例外的導入
         : 1音増加と新音色
    1990 : スーパーファミコン
         : SPC700 / S-DSP
         : 8 voice BRR, 32kHz級, echo/FIR
    1994 : PlayStation
         : SPU 24 voice, 44.1kHz ADPCM
         : CD-DA / XA streaming
    1997 : PSY-Qワークフロー確立
         : VAG / VAB / SEQ
...
9. **『ファイナルファンタジーVII』（1997年）** — PSY-Q資料は、SEQによる **MIDI的シーケンス再生** が「少数の楽器サンプルで長い楽曲を効率よく鳴らせる」と説明している。『ファイナルファンタジーVII』はこの長所を最大限に使った代表作で、PlayStation時代の"豪華な音"のかなりの部分は、実際には **大量の生波形ではなく、慎重に作られたVAGサンプル群とシーケンス** で成立していた。これによってSPU RAMを節約しつつ長大なスコアを保持できた点が、PlayStation時代のゲーム音楽制作を象徴する。公式音源はスクウェア・エニックスの公式オリジナル・サウンドトラックで入手できる。
...
```mermaid
flowchart TB
    subgraph SW2[Nintendo Switch 2 公開アーキテクチャ]
        S1[ゲームエンジン / ミドルウェア]
        S2[システムミキサー / Virtual Surround]
        S3[HDMI出力 5.1ch Linear PCM]
        S4[本体スピーカー / ヘッドホン]
        S5[内蔵マイク]
        S6[音声処理専用チップ]
        S1 --> S2
    ...
    subgraph PS5[PlayStation 5 公開アーキテクチャ]
        P1[ゲームエンジン / ミドルウェア]
        P2[Tempest 3D AudioTech]
        P3[ヘッドホン]
        P4[TVスピーカー仮想サラウンド]
        P5[Dolby Atmos対応HDMI機器]
        P1 --> P2
        P2 --> P3
        P2 --> P4
        P2 --> P5
    ...
    subgraph XSX[Xbox Series X/S 公開アーキテクチャ]
        X1[ゲームエンジン / ミドルウェア]
        X2[Custom audio hardware]
        X3[Microsoft Spatial Sound]
        X4[Windows Sonic / Dolby Atmos / DTS X]
        X1 --> X2 --> X3 --> X4
    end
```
