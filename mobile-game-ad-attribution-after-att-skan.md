...
```
# 仕上げAIへ図表制作依頼
ATT前後の広告アトリビューションを左右比較する概念図を作成してください。左は「広告接触→IDFAで一致→インストール→継続・広告視聴→ユーザー単位LTV→入札」、右は「広告接触→ATT拒否→直接一致できない→SKANの集約ポストバック＋自社集計＋モデル→入札」としてください。「自社アプリ内の行動データまで消えるわけではない」ことが誤解なく伝わる構成にしてください。数値グラフではありません。
```
...
導入直後の数字は、ゲームジャンルによって備えが均一ではなかったことを示す。AppsFlyerが2021年5月17日から24日までを含む1,590アプリのデータで公表した週次調査では、全ジャンルのATT実装率は18%、ATTダイアログに対するグローバルのオプトイン率は40%だった。ゲームのATT実装率は、ハードコア29%、ソーシャルカジノ29%、ミッドコア23%に対し、ハイパーカジュアル17%、カジュアル16%であった。[[4](#ref-4)]
...
```
# 仕上げAIへ図表制作依頼
AppsFlyerの2021年5月調査を横棒グラフにしてください。ゲームのATT実装率をハードコア29%、ソーシャルカジノ29%、ミッドコア23%、ハイパーカジュアル17%、カジュアル16%の順で表示し、別枠の注記として「全アプリ実装率18%」「グローバルATTオプトイン率40%」を示してください。実装率とオプトイン率を同じ系列として比較しないでください。出典表記は「出典：AppsFlyer Japan、2021年5月31日」。正確な数値グラフなのでEChartsを使用してください。
```

この表のジャンル別数値は **オプトイン率でも、被害の大きさでもなく、アプリ側の実装率** である。したがって「ハイパーカジュアルの影響が17%だった」と読むのは誤りだ。AppsFlyer自身は、ユーザー情報に依存する広告収益の比重が大きいハイパーカジュアルで実装が進んでいない状況を指摘していた。広告収益依存ジャンルほど、IDFAを失ったときにターゲティング、広告単価、獲得元別LTVの三方から影響を受けやすいという構造的な脆弱性があったのである。[[4](#ref-4)]
...
```
# 仕上げAIへ図表制作依頼
SKAN 4.0の3つのコンバージョン期間と送信遅延を示す横長タイムラインを作成してください。第1期間0〜2日→24〜48時間後、第2期間3〜7日→24〜144時間後、第3期間8〜35日→24〜144時間後と正確に記載してください。第1はfineまたはcoarse、第2・第3はcoarseのみ、Tier 0は第1のみという注記も入れてください。期間と遅延を混同しない構成にしてください。
```
...
```
# 仕上げAIへ図表制作依頼
iOSとAndroidのプライバシー規制の歩みを対比する横型タイムライン図を作成してください。上段（iOS）：「2021年4月26日 ATT義務化」→「以後、SKANを中心とした集約計測が定着」。下段（Android）：「2022年2月16日 Privacy Sandbox on Android発表（2年間は既存機能を支援）」→「2025年10月17日 Attribution Reporting API等を廃止」。両者が異なる結末に分岐したことが一目で分かる構成にしてください。数値グラフではなく概念的な年表です。
```
