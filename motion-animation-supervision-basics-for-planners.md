---
description: "ゲーム内のキャラクターが、モデルの形を保ったまま動いたとしても、それだけで監修完了にはならない。攻撃の意図が一目で分かるか、踏み込んだ足に重さがあるか、プレイヤーの入力に対して待たされすぎないか、次の状態へ自然に移れるかまで確認して、初めて「動きが成立した」と言える。"
...
![均等な速度と、ためて急加速する動作のスペーシング比較](image/animation-spacing-comparison.webp)
...
![Sync Groupを使わない歩行と走行のブレンド例](image/unreal-sync-group-off.webp)

![Sync Groupを使った歩行と走行のブレンド例](image/unreal-sync-group-on.webp)

出典：[Animation Sync Groups in Unreal Engine｜Epic Developer Community](https://dev.epicgames.com/documentation/en-us/unreal-engine/animation-sync-groups-in-unreal-engine) に掲載されている公式画像を、比較説明に必要な範囲で無改変引用。
...
UnityのAnimation Transitionは、パラメーター条件やExit Timeを使って状態を切り替え、状態間のブレンド時間も設定できる。[[4](#ref-4)] Unreal EngineのAnimation Montageも、セクションをゲームロジックや入力によって動的に切り替えられる。[[5](#ref-5)] これは、ゲーム用アニメーションが一本の映像を再生するだけでなく、状態機械と入力に接続された部品であることを示している。
...
![攻撃アニメーションとゲーム判定を照合するタイムライン](image/animation-hitbox-timeline.webp)
