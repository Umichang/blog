...
## 図表の目的
『ドンキーコング バナンザ』の「自由度を守るQA」が、単なる終盤テストではなく、設計・制作・検証をつなぐ共同作業であることを示す。

## 構成
横長の明快な3層図。中央に大きく「守る価値：息を吸うように破壊できる自由」と置く。
上段は「企画・レベル設計」から中央へ「シーケンスを飛ばしても破綻しない仕様」をつなぐ。
左下は「アーティスト・プログラマー」から中央へ「通常時の負荷を下げる最適化」をつなぐ。
右下は「マリオクラブのテスター・自動テスト」から中央へ「想定外の操作を予測して検証」をつなぐ。
中央から下部の成果として「安心して破壊できる」「制約を増やさない」「面白いなら活かす」の3つの短い札を並べる。

## 表記・見た目
図中の文字は上記の日本語だけを用いる。記事内の見出し番号や「Fig.」は入れない。淡い黄土色、深い茶色、落ち着いた青緑を基調にし、ゲーム画面やキャラクターの模写は使わない。余白を十分に取り、スマートフォンでも日本語を読める大きさにする。
```
...
本作のQAを象徴するのが、レースの一部を力技で飛ばして次のマップへ進む、タイムアタック的な操作の例である。発見時にはバグだったが、面白い体験になり得ると判断され、後のセリフ差分まで用意したうえで仕様に組み込まれた。[[2](#ref-2)]
...
<a id="ref-1"></a>1. [CEDEC2026公式サイト「CEDEC AWARDS　最優秀賞・特別賞」][1] - 『ドンキーコング バナンザ』開発チームのエンジニアリング、ゲームデザイン、ビジュアルアーツ各部門の最優秀賞と評価内容。
...
[1]: https://cedec.cesa.or.jp/2026/event/awards/prize/
