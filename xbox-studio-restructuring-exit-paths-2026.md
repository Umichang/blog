...
```
# 仕上げAIへ図表制作依頼
16:9の横長インフォグラフィックを作成する。日本語の読みやすいゴシック体を用い、背景は白、アクセントはXboxを想起させる緑と、注意を示す黄土色に限定する。
タイトルは「スタジオ再編の3つの出口」。中央に「親会社のポートフォリオ見直し」の箱を置き、右方向へ3本の矢印を伸ばす。
上段は緑の「独立」カード。小見出しは「IP・カタログ・次作資金を引継ぐ」。要点は「チームが自ら資金と販売先を決める」「旧作収益が次作までの時間を支える」。
中段は青緑の「売却」カード。小見出しは「新しい親会社が完成資金を引継ぐ」。要点は「開発中タイトルの完成を優先」「予算・発売条件は買い手の方針に左右される」。
下段は黄土色の「閉鎖」カード。小見出しは「開発ラインが停止する」。要点は「IPは保有者に残り得る」「引継ぎ先がなければ作品は休眠する」。
各カードの下に小さく判断要因として「カタログの収益性」「開発中タイトルの進捗」「追加投資の回収見込み」「チーム規模と移管しやすさ」を並べる。人物の顔や企業ロゴは使わない。文字量を厳守し、装飾より構造の明瞭さを優先する。
```
...
<a id="ref-1"></a>1. [Microsoft cuts 4,800 jobs, including many at Xbox in a 'reset' of its gaming division][1] - AP通信による、Microsoft全体の約4,800人とXboxの当日1,600人・年度内3,200人計画の報道。

[1]: https://apnews.com/article/5a8f712c531911089dee008b3bbb33c4
