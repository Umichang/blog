...
# 仕上げAIへ図表制作依頼
🔴必須
『モグリアテール』公式サイトのゲームフローを基に、多段抽選の構造を横長の工程図で示す。
「ピンク／パープルボールを落とす」→「ボーナスプッシャーに挑戦」→「中央からビッグボールを手元へ引き込む」→「ビッグボールを落とす」→「ジャックポットチャレンジ」→「大量メダル獲得」の順に並べる。
各段に「通常盤面の物理」「中央機構の物理」「目標物の輸送」「最終抽選」「払い出し」と役割を短く添え、押し出しが報酬の出口であると同時に次の抽選への入口であることを強調する。
固有筐体の外観を再現する図ではなく、記事内容を整理した概念図とする。
```
...
# 仕上げAIへ図表制作依頼
🟡推奨
セガ特許JP2010088830Aの実施形態を基に、スキル判定から初心者救済へ至るフロー図を作る。
「所定時間の投入枚数・入賞枚数を計測」→「入賞率＝入賞枚数／投入枚数」→「想定値50％と比較」→「下回る場合は初心者レベルと判定」→「ジャックポット確率／ポイント加算量／払い出し枚数のいずれかを引き上げる」の主経路を示す。
別経路として「想定値に届かなかった分を内部メモリへ貯留」→「ジャックポット当選時に一部を上乗せ」を添える。
図中に「特許に記載された構成例であり、全機種の共通仕様ではない」と明記する。
```
...
物理をデジタルで再現する代わりに、現実の物理現象を撮影して配信する道もある。タイトーオンラインメダルの『マーブルレース』は、8色のボールが物理コースを走る順位を予想し、ゲーム内専用メダルをベットするゲームとして、2024年7月29日にサービスを開始した[[8](#ref-8)]。開発にはエクストリームが参加し、同社は『マーブルレース』の開発とタイクレとの連携を支援した実績を公開している[[9](#ref-9)]。

プレイヤーはレース前に予想してベットし、物理的に転がるボールの順位変化を観戦する。カメラの向こうのボールや発射装置を操作する仕組みではなく、実機のクレーンをプレイヤーが遠隔操作するオンラインクレーンゲームとは体験の構造が異なる。

タイトーとエクストリームの公開事例によれば、結果だけでなく抽選過程を見て楽しめることが企画の中心に置かれた。コースの途中に死角ができないよう複数のカメラを配置し、賭けたボールが先頭から最下位へ落ちるような過程も追えるようにした。抽選式ゲームでは「裏で操作されているのではないか」という疑念が生じ得るため、物理的な入れ替わりを一続きの映像として見せることが、納得感と信頼を支える[[10](#ref-10)]。

映像を見せるだけでは運営システムにならない。高速で通過するボールの色と時刻を機械が判定し、カメラ切り替えや実況へつなぐ必要がある。開発事例では、ボール通過時の画像へ色のラベルを付け、何万枚もの写真をAIの学習データとして蓄積したと説明されている。照明、影、コース面の反射で同じ色の見え方が変わるため、当初12色だったボールは、識別の難しさと当選確率の低下を踏まえて8色へ減らされた[[10](#ref-10)]。
...
オンラインでの展開は一つではない。コナミの『コナステ メダルコーナー』は、アーケードのメダルゲームをPCやスマートフォンで遊べるようにし、プレイ用メダルをウェブの購買部やアプリ内で購入できるサービスを提供している[[11](#ref-11)]。サミーネットワークスの『GAPOLI』は、多数のメダルゲーム、ぱちんこ・パチスロ、カジノゲーム等をまとめたオンラインゲームセンターであり、ゲームで得たコインをGAPOLIモールの商品値引きへ使える[[12](#ref-12)]。公式の現行説明は「商品との直接交換」ではなく、100コインを1円相当の値引きに利用する仕組みである。
...
<a id="ref-9"></a>9. [受託開発実績｜エクストリーム][9] - 『マーブルレース』の開発とタイクレ連携を支援したことを示す開発会社の実績ページ。

<a id="ref-10"></a>10. [緊密な連携から生まれた「見ていて楽しい」オンラインメダルゲーム｜エクストリーム][10] - 複数カメラ、AIによる色認識、何万枚もの学習画像、リアルタイム観戦と抽選への信頼を扱うタイトー・開発会社の公開事例。

<a id="ref-11"></a>11. [e-amusementサイト「購買部」へのPayPay導入｜コナミアミューズメント][11] - 『コナステ メダルコーナー』用メダルをウェブの購買部で購入できることを示す公式発表。

<a id="ref-12"></a>12. [GAPOLI プレイガイド｜サミーネットワークス][12] - オンラインゲームセンターで得たコインを、GAPOLIモールの商品値引きへ利用できることを示す公式案内。
...
[8]: https://www.taito.co.jp/en/taito-prize/topics/26481
[9]: https://www.e-xtreme.co.jp/development/
[10]: https://www.e-xtreme.co.jp/case/taitoonlinemedal/
[11]: https://www.konami.com/amusement/corporate/ja/news/release/20250819/
[12]: https://portal.gapoli.net/playguide/
