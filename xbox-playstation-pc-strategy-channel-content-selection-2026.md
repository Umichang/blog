...
```
# 仕上げAIへ図表制作依頼
Xboxの観測とPlayStationの方針転換で、情報の裏付けが積み上がった速度と強さの違いを対比する概念図（横に並べた2本のタイムライン）を作成してください。

左側「Xbox」：
- 2026年8月1日：Jez Corden氏（Windows Central）がXbox Twoポッドキャストでリスナーの質問に回答（仮定のシナリオへの回答）
- 2026年8月2日：複数ニュースサイトが「Steam離脱を検討」という観測として報道
- 2026年8月3日：Corden氏本人が文脈を訂正し、Steam離脱の検討を明確に否定

右側「PlayStation」：
- 2026年3月4日：Jason Schreier氏（Bloomberg）が、自社開発ナラティブ型シングルプレイ作品のPC展開後退を報道（匿名情報源）
- 2026年5月18日：Hermen Hulst氏（SIE Studio Business Group CEO）が社内タウンホールで同内容を従業員へ伝達（Schreier氏が報道、独立した裏付け）
- 2026年6月18日：ソニー年次報告書から該当文言が削除／SIE CEO西野秀明氏が『ファミ通』で公式に方針を説明（公式資料・公式発言）

意図：左は「同一人物が数日で自己修正して終わった観測」、右は「異なる種類の裏付け（匿名報道→社内発言の外部報道→公式資料→公式インタビュー）が2か月半かけて積み上がった経緯」という、裏付けの質と量の違いを一目で対比させてください。年表形式でも、上下に並べた2本のタイムラインでも構いません。数値グラフではなく概念図として扱ってください。
```
...
```
# 仕上げAIへ図表制作依頼
PlayStationのPC（Steam）版売上について、算出元の異なる2つの第三者推定を比較する棒グラフを作成してください。

- VG Insights系推定：集計期間2020年8月〜2024年6月、PC累計売上 約8.3億ドル（うちHelldivers 2が約34%）。出典：Game World Observer（https://gameworldobserver.com/2024/06/07/playstation-pc-revenue-830-million-game-sales-steam）
- Alinea Analytics系推定：集計期間はより長期（2026年時点まで含む、正確な集計開始時点は出典に明記なし）、総売上（グロス）約15億ドル、Valve手数料控除後（ソニー取り分相当）約12億ドル。出典：Alinea Analytics公式ブログ（https://alineaanalytics.com/blog/steamps/）

必須の注記：
- 2系統の推定は集計期間・算出方法が異なるため、同じ物差しで比較できないことをグラフ内または直下の注記で明示してください（「対立する数値ではなく、異なる期間・手法による推定」という趣旨）。
- いずれもソニー公式の監査済み数値ではなく、第三者アナリティクス企業による推定である旨を明記してください。
- 不明な項目（Alinea推定の正確な集計開始時点など）は、数値を推測で埋めず空欄のままにしてください。
```
...
```
# 仕上げAIへ画像引用依頼
Xbox Wireの公式記事「From GDC: Building the Next Generation of Xbox」（2026年3月11日、Project Helix発表記事）のヘッダー画像またはProject Helix関連のキーアートを引用候補としてください。
URL: https://news.xbox.com/en-us/2026/03/11/project-helix-building-next-generation-of-xbox/
著作権法上の引用要件（必要な範囲・主従関係・出典明記・無改変）を満たす形で使用し、満たせない場合はスキップしてください。掲載する場合は出典元（Xbox Wire、記事URL）を明記してください。既存記事「Xbox再編から考える、スタジオの「独立・売却・閉鎖」はなぜ分かれるのか」で使用した「Resetting XBOX」記事の画像とは別の画像です。
```
...
```
# 仕上げAIへ図表制作依頼
本文5-1節の表（判断軸／確認する問い／近い事例）を視覚化した概念図を作成してください。

- 軸1「配信チャネルの主導権」：どのストア・ランチャー・アカウント連携・購入導線を許容/統合するか。事例としてXboxのSteam継続とXbox PC・Battle.net・各社ランチャーとの接続。
- 軸2「コンテンツ選別」：どの作品を、どの時点で、どのハードへ出すか。事例としてPlayStationの自社開発シングルプレイとライブサービスの二本立て。

2つの判断軸が別物であることが視覚的にわかる構成（左右対比、または2本の独立した矢印など）にしてください。数値グラフではなく概念図として扱ってください。
```
