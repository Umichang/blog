...
```
# 仕上げAIへ図表制作依頼
「操作できる映画」を中央に置き、そこから「ペーシング」「トラバーサル・ガンプレイ」「敵AIのTAPs」「操作可能なセットピース」の4要素へ線を伸ばす概念図を作成する。各要素から中央へ戻る短い説明として、順に「操作強度で緩急を作る」「移動と射撃を切らない」「垂直経路を敵と共有する」「崩壊中も入力を保つ」を配置する。4要素を並列の機能一覧ではなく、同じ設計目標を支える相互接続した構造として見せる。日本語、横長1600×1000ピクセル、記事表示幅でも文字が読める構成とする。
```
...
```
# 仕上げAIへ画像引用依頼
よじ登りながら射撃する「トラバーサル・ガンプレイ」の場面を写したスクリーンショットを引用候補として検討してほしい。Naughty DogまたはSony公式のプレスキットや紹介ページに掲載されている画像を対象とし、著作権法上の引用要件（必要な範囲・主従関係・出典明記・無改変）を満たすものに限る。要件を満たす画像が見つからない場合はスキップしてよい。
```
...
```
# 仕上げAIへ図表制作依頼
TAPs（Traversal Action Packs）による「垂直経路の共有」を示す概念図を依頼する。同じ壁・はしご・足場の登攀経路に、プレイヤーと敵の双方がアクセスできることを視覚化し、高所が「固定的な安全地帯」ではなく「条件付きの優位」に変わる様子を示す。例えば、プレイヤーが高所へ登る矢印と、敵が同じ経路を追って登る矢印を並べて描き、「留まりすぎると敵も登ってくる」という本文の論点を表現するとよい。数値は不要で、経路共有という概念的な関係性のみを表現すること。
```
...
```
# 仕上げAIへ図表制作依頼
セットピースにおける「事前計算とリアルタイム処理」の三層構造図を依頼する。下から順に、①Mayaで事前シミュレーションしベイクした崩壊アニメーション（建物がどの順番・速度で倒れるかを決める層）、②その上に重なるリアルタイム物理（モニターや植物などの反応）、③さらに重なる粒子表現（火花や瓦礫など）、という三層を積み重ねた断面図・レイヤー図のような構成が考えられる。各層がどちらの目的（作り手が保証したいタイミングの確実さ／プレイヤーが必要とする応答性）を担うかも短い説明で示すとよい。
```

```
# 仕上げAIへ画像引用依頼
崩落するホテル、または走行中の列車内での戦闘シーンを写したスクリーンショットを引用候補として検討してほしい。Naughty DogまたはSony公式のプレスキットや紹介ページに掲載されている画像を対象とし、著作権法上の引用要件（必要な範囲・主従関係・出典明記・無改変）を満たすものに限る。要件を満たす画像が見つからない場合はスキップしてよい。
```
