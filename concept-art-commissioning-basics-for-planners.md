...
```
# 仕上げAIへ図表制作依頼
コンセプトアート発注の一連の流れを示すフローチャートを作成してください。
本文の第2章「発注書とオリエンテーション資料を設計する」の内容を視覚的に要約する図とします。

【流れ】
1. 目的の言語化（この絵で何を決めるか）
2. 必須・狙い・避けるの三層整理
3. 参考画像・ムードボードの収集
4. 発注書の作成（背景・目的・対象・成果物・進行）
   ↓
5. オリエンテーション（目的・必須条件の共有、相手からの理解の言い返し）
   ↓
6. サムネイル・ラフ（シルエット・ポーズ・構図を確認）
   ↓
7. 線画（形状・プロポーション・装備の構造を確認）
   ↓
8. 着彩・仕上げ前（色・明暗・視線誘導を確認）
   ↓
9. 最終確認（用途・サイズ・納品形式・派生展開を確認）

各チェックポイント（5〜9）の枠内に、それぞれの段階で「見るもの」を短く添えてください
（本文中の表と対応させること）。社内デザイナー・外部クリエイターのどちらの発注にも
使える図であることが伝わるよう、フロー全体に共通の見出しを付けてください。
色分けの目安：準備段階（1〜4）を青系、レビュー段階（5〜9）を緑系にしてください。
```
...
```
# 仕上げAIへ図表制作依頼
本文中の「使える語彙を持つ」の表（シルエット・プロポーション・線の情報量・色数彩度・
視線誘導・余白の使い方）を、架空のゲームキャラクターのシルエット・線画イラスト一枚に
重ねて図解してください。

【表現内容】
- 中央に、武器を持つ汎用的なキャラクターのシンプルなラフイラスト（線画程度の密度）を配置する。
- イラストの各部位から引き出し線を伸ばし、以下の観察ポイントを短いラベルで示す。
  - 肩・武器の外周（シルエット）
  - 頭部と胴の比率（プロポーション）
  - 顔周辺の線の密度（線の情報量）
  - 全身の彩度分布（色数・彩度）
  - 顔から手元への明暗差（視線誘導）
  - 上部・周辺の空きスペース（余白の使い方）
- 特定の実在キャラクターや作品を想起させるデザインは避け、あくまで観察ポイントを
  説明するための汎用的なシルエットとすること。

この図は、フィードバックを伝える際に「どこを」「どちらへ」動かしたいかを示す
本文中の実例（引き出し線・矢印・トリミング枠の使用）とも一致するデザインにしてください。
```

```
# 仕上げAIへ画像引用依頼
Interaction Design Foundation（IxDF）の「What are Design Critiques?」ページに掲載されている、
クリップボードの周りに吹き出しが並んだイラスト（キャプション：
「© Interaction Design Foundation, CC BY-SA 4.0」と明記された画像）の引用を検討してください。
このイラストはCC BY-SA 4.0で明示的にライセンスされていることをページ上で確認済みです。
ただし、実際に埋め込む前に最新のライセンス表記・使用条件を再確認し、CC BY-SA 4.0の
継承要件（同一ライセンスでの再配布）を本ブログのMITライセンス表記と併記する形で
問題なく満たせるかを確認してください。満たせない場合はスキップする旨を明記してください。
出典表記例：「出典：Interaction Design Foundation『What are Design Critiques?』
