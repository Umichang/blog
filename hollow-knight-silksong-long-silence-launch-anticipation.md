...
```
# 仕上げAIへ画像引用依頼
Hollow Knight: Silksongの公式キービジュアル（主人公ホーネットを描いたアートワーク）の引用を依頼する。優先度🟡推奨。
記事冒頭で作品の主題を視覚的に示す目的で使用する。Team Cherry公式サイト（teamcherry.com.au）またはSteamストアページ掲載の公式素材を候補とする。著作権法上の引用要件（必要な範囲・主従関係・出典明記・無改変）を満たす画像を採用し、出典（Team Cherry公式、掲載元URL）を明記すること。画像の具体的な検索・選定は仕上げAI側で行ってよい。
```

```
# 仕上げAIへ図表制作依頼
本記事全体の骨格となる7年間の経緯タイムライン図の作成を依頼する。優先度🔴必須。
以下のマイルストーンを時系列に並べること。
・2017年2月24日：前作『Hollow Knight』PC版発売
・2019年2月14日：Silksong正式発表（当初DLCとして構想、フル続編へ拡大）
・2019年末以降：開発陣による定期的な進捗報告が途絶える（沈黙期間の開始）
・2022年6月：Xboxショーケースで「今後12か月以内」の発売を示唆（結果的に未達成）
・2025年8月21日：BloombergのインタビューでGibson・Pellen両氏が沈黙の理由を説明
・2025年9月4日：発売、Steam・eShop・PSN・Microsoft Storeで購入障害が発生
・2025年12月15日：Team Cherryが購入者700万人超を公式発表
沈黙期間（2019年末〜2025年8月）が全体の大部分を占める視覚的な長さで表現し、発売日の「好意的な殺到」が終着点であることを強調するデザインが望ましい。
```
...
発売時刻の直後、Steamではストアへの接続・カート・購入処理が不安定になり、`E502 L3` というエラーも広く確認された。Nintendo eShop、PlayStation Store、Microsoft Storeでも購入ページの表示、購入、ダウンロードに関するエラーや障害が報じられた。各社で同じエラーコードが公式に統一されていたわけではないため、Steam以外のコードを一つの確定事実として列挙することは避けるべきである。確認できるのは、四つのストアで同時多発的に購入導線が機能しにくくなったことだ。[[1](#ref-1)][[8](#ref-8)]

Steamでは発売45分以内に同時接続者数が10万人を超えた。初日のピークは535,213人と報じられ、その後の週末、SteamDBが記録する最高同時接続者数は587,150人に達した。ここでいう同時接続者数は、ある瞬間にSteamでゲームを起動していた人数であり、販売本数でも全機種のプレイヤー数でもない。数字の用途を混同しないことが重要である。[[1](#ref-1)][[9](#ref-9)][[10](#ref-10)]
...
```
# 仕上げAIへ図表制作依頼
Steam同時接続者数が発売直後に急上昇した様子を示す折れ線または棒グラフの作成を依頼する。優先度🟡推奨。
以下の4点をプロットすること。
・発売10分後：75,000人
・発売45分後：100,000人超
・発売初日ピーク：535,213人
・発売weekend最高値（2025年9月6日）：587,150人
本文の表と重複する内容だが、急激な立ち上がりを視覚的インパクトとして伝える目的で、グラフ形式での補完を意図している。横軸は経過時間（分・日単位で目盛りを切り替えてよい）、縦軸は同時接続者数とすること。
```

```
# 仕上げAIへ図表制作依頼
「3日間で500万人」と「700万本超」という2つの数字を混同しないための整理図の作成を依頼する。優先度🟡推奨。
販売本数（Steam3日間で300万本超）、全機種プレイヤー数（3日間で500万人超、うちXbox Game Pass経由が約100万人）、公式累計購入本数（2025年12月時点で700万本超）という3種類の異なる指標の関係を、それぞれの定義・出所・時点の違いが分かる形（階層図または注記付きの比較ブロック）で示すこと。同時接続者数（発売初日ピーク535,213人）も参考指標として含めてよいが、販売本数・プレイヤー数とは性質が異なる旨を明記すること。KPIの読み違いを防ぐ実務教材としての利用を想定している。
```
...
```
# 仕上げAIへ図表制作依頼
『No Man's Sky』とSilksongを対比するインフォグラフィックの作成を依頼する。優先度🟡推奨。
左右2列の対比形式で、以下の流れを示すこと。
左列（No Man's Sky）：曖昧な機能公約の積み重ね→過大な期待の形成→発売直後の大炎上→9年かけた無料アップデートによる信頼回復
右列（Silksong）：公約を増やさない沈黙の維持→期待が減衰しないまま持続→発売時の好意的な殺到（ストアクラッシュ）→高評価での着地
両者の分岐点が「発売時点の機能と、事前に増幅された期待との落差の有無」にあることが一目で伝わるデザインが望ましい。本文4章の論旨を視覚的に補強する目的で使用する。
```

```
# 仕上げAIへ画像引用依頼
発売日にSteam・eShop・PSN・Microsoft Storeで発生した購入障害を示すスクリーンショットの引用を依頼する。優先度🟢任意（存在すれば採用、見つからなければスキップ）。
GameSpot記事等に掲載された、Steamのエラー画面のスクリーンショットを候補とする。著作権法上の引用要件（必要な範囲・主従関係・出典明記・無改変）を満たす画像が現存しない場合は、その旨を明記した上でこの依頼をスキップすること。採用時は出典（メディア名・記事URL）を明記すること。画像の具体的な検索・選定は仕上げAI側で行ってよい。
```

```
# 仕上げAIへ画像引用依頼
沈黙期間中にファンコミュニティで広まった「silkposting」文化（道化師の化粧、儀式めいた投稿等）を示す画像の引用を依頼する。優先度🟢任意（存在すれば採用、見つからなければスキップ）。
著作権者が個人ユーザーである可能性が高く、引用要件（出典明記・著作権者の把握・無改変）を満たす画像を見つけるのが難しい可能性がある。要件を満たす画像が見つからない場合は、無理に採用せずスキップする旨を明記すること。
```
