---
description: "東亜プランは1994年に倒産した。しかし、その後に残ったのは空白ではない。ケイブ、ライジングなどへ分かれたスタッフは、同じ設計上の問いにそれぞれ異なる答えを出した。本稿は、アーケードシューティングの名門を組織史として読み直す。"
---
...
```
# 仕上げAIへ図表制作依頼
「撃って避ける」を学習可能にする設計思想を、反復ループとして示す概念図を作成してください。
「挑戦する」→「失敗する」→「原因が分かる（敵出現・弾道・危険地点の因果）」→「次回の判断を修正する」→「再挑戦する」という循環を、円環状の矢印で示してください。
訴求点は「短いプレイの反復が、単なる難度調整ではなく、次の1プレイへの動機を作る設計になっている」という点です。数値グラフではなく、循環構造を示す概念図としてください。
```

```
# 仕上げAIへ画像引用依頼
『タイガーヘリ』または『BATSUGUN』のアーケード版ゲーム画面スクリーンショットを引用したい。TOAPLAN TITLES（TATSUJIN運営の東亜プラン公式サイト）に掲載されている画像を候補としてほしい。訴求点は「撃って避ける」ストロングスタイルの実際の画面を視覚的に示すこと。著作権法上の引用要件（必要な範囲・主従関係・出典明記・無改変）を満たせる素材が見つからない場合はスキップし、その旨を明記すること。出典明記の際はTATSUJIN Co., Ltd.のクレジットを明記すること。
```
...
```
# 仕上げAIへ図表制作依頼
「東亜プランらしさ」を構成する要素をマップ状に整理した概念図を作成してください。
中央に「一見して東亜プラン作品と分かる濃さ」を置き、周囲に「軍用機・戦車・異形の巨大ボスなど即座に役割を読ませるデザイン」「作品共通のフォント・インストラクションカードのデザイン」「終盤で緊張感を高めるBGM」「弾封じを許容する仕様」を配置してください。
訴求点は「これらの要素が個別の美術の好みではなく、同じ操作感の約束を補強する設計上の一貫性である」という点です。数値グラフではなく、要素の関係性を示す概念図としてください。
```
...
```
# 仕上げAIへ図表制作依頼
東亜プランを起点とした組織の分岐と継承を示す樹形図（組織系譜図）を作成してください。
中央上部に「東亜プラン（1994年倒産）」を置き、そこから下方向へ「ケイブ（1994年設立）」と「ライジング」の2本の枝を伸ばしてください。ライジングの枝はさらに「株式会社エイティング（2000年10月吸収合併）」へ接続してください。
別の枝として、東亜プランから現在の版権管理会社「TATSUJIN」へ矢印を伸ばし、そこからさらに「Embracer Group（2022年8月傘下入り）」へ接続してください。
訴求点は「一つの組織が消滅した後、人材の系譜と権利の系譜という異なる2種類の継承ルートが並行して存在する」という点です。数値グラフではなく、組織間の関係を示す系譜図としてください。タクミコーポレーションとガゼルは本文で個別の年代・接続関係を確認できていないため、図中には含めず、上記の確認済みの関係のみを図示してください。
```
...
```
# 仕上げAIへ画像引用依頼
新作『TATSUJIN EXTREME』のイメージ画像またはゲーム画面キャプチャを引用したい。TATSUJIN公式サイトまたは4Gamer等の紹介記事に掲載されている画像を候補としてほしい。訴求点は、レトロIPが現代的な新作として再解釈されている様子を視覚的に示すこと。著作権法上の引用要件（必要な範囲・主従関係・出典明記・無改変）を満たせる素材が見つからない場合はスキップし、その旨を明記すること。出典明記の際はTATSUJIN Co., Ltd.のクレジットを明記すること。
```
