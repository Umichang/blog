...
# PS2世代ゲームデザイン集 #7：『ペルソナ3』は18年・4バージョンで何を変え、何を変えなかったか――再解釈と追加販売を設計として読む
...
```
# 仕上げAIへ図表制作依頼
『ペルソナ3』原作の「時間資源」概念図を制作する。左から右へ「昼：学園生活・コミュニティ」「夜：タルタロス探索・能力育成」「満月：大型シャドウ戦」の3段階を置き、昼の人間関係がペルソナ合体の強化を通じて夜の戦力へつながり、夜の探索成果が満月戦の準備になる循環を矢印で示す。日常と戦闘を別々の箱として断絶させず、限られた一日の選択が双方に効くことを日本語で簡潔に表す。数値グラフではないため、gpt-imageで作成する。画像内に作品ロゴ、キャラクター画像、細かな固有名詞は使わない。
```
...
```
# 仕上げAIへ図表制作依頼
『ペルソナ3 フェス』の通常版とアペンド版の税込希望小売価格を比較する棒グラフを制作する。数値は通常版8,190円、アペンド版5,040円の2本のみ。出典はファミ通.com「スペシャル企画『ペルソナ3 フェス』」（2007年、https://www.famitsu.com/sp/070319_p3f/）。アペンド版の棒グラフには「初回起動時に『ペルソナ3』ディスクの認証が必要」という注記を小さく添える。これは2007年当時の価格であり、現在の実勢価格ではないことも注記する。Apache EChartsで正確な数値グラフとして作成する。
```

```
# 仕上げAIへ図表制作依頼
2006年から2024年までの『ペルソナ3』4主要バージョンの横長年表を制作する。順に「2006　ペルソナ3」「2007　ペルソナ3 フェス」「2009　ペルソナ3 ポータブル」「2024　ペルソナ3 リロード」を配置し、2024年9月の「Episode Aegis」配信をリロードから伸びる有償DLCの枝として示す。各版の要点は短く、原作＝昼夜・コミュニティ・タルタロス、フェス＝THE ANSWER・アペンド版、ポータブル＝女性主人公・2D移動、リロード＝3D再構築・発売時は両追加要素なし、とする。「各版に排他的な要素が残る」という関係を、版ごとに異なる色の小さなラベルで明瞭にする。数値グラフではないためgpt-imageで作成する。画像内に作品ロゴやキャラクター画像は使わない。
```
...
```
# 仕上げAIへ図表制作依頼
『ペルソナ3 フェス』『ペルソナ3 ポータブル』『ペルソナ3 リロード』の追加・提供方式を比較する概念図を制作する。3列構成とし、フェスは「後日談 THE ANSWER」「通常版／原作ディスク認証のアペンド版」「既存ユーザーへの低い追加負担」、ポータブルは「日常移動の2D化」「女性主人公ルート」「携帯機向けに体験を組み替え」、リロードは「3D空間の再構築」「Episode Aegisは本編後の有償DLC」「女性主人公は非搭載」を示す。フェスを無条件に善、リロードを無条件に悪と見せず、各方式の利点と残る課題を対称的に表現する。数値グラフではないためgpt-imageで作成する。作品ロゴ、キャラクター画像、価格の記載は使わない。
```
...
```
# 仕上げAIへ画像引用依頼
『ペルソナ3』（2006年）、『ペルソナ3 フェス』（2007年）、『ペルソナ3 ポータブル』（2009年）、『ペルソナ3 リロード』（2024年）の4パッケージアートを横並びで比較できる形で引用したい。アトラス／セガの公式製品画像を優先し、著作権法上の引用要件（必要な範囲・主従関係・出典明記・無改変）を満たすものに限る。要件を満たす画像が揃わない場合は、無理に代替せずスキップする。出典は各版の発売元・発売年を明記する。
```
