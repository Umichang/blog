...
```
# 仕上げAIへ図表制作依頼
サンプルサイズとMDEの非線形な関係を示す棒グラフを制作する。有意水準は両側5%、検出力80%、基準率20%、1対1割り当て。横軸は「MDE：絶対値2ポイント」「MDE：絶対値1ポイント」、縦軸は「1群あたりの必要人数」。値はそれぞれ6,510人、25,583人とし、値を変更・補完しない。効果量が半分になると必要人数が約3.9倍になることを注記する。
```
...
SRMを検出したら、効果指標を解釈する前に、次の順で原因を追う。Microsoftの研究は、原因を割り当て、実行、ログ処理、分析の各段階に分類している。[[4](#ref-4)]
...
```
# 仕上げAIへ図表制作依頼
SRM診断の工程図を制作する。開始は「期待比率と観測人数をカイ二乗検定」、分岐は「SRMなし→効果指標の確認へ」「SRMあり→効果指標の判断を保留」。SRMありの先を「割り当て」「実行」「ログ処理」「分析条件」の4段階に並べ、最後を「原因修正後に再集計・必要なら再実験」とする。日本語はこの短い語句だけを使い、医療や飛行機の比喩は入れない。
```
...
```
# 仕上げAIへ図表制作依頼
横軸を「初回接触からの時間」、縦軸を「Aに対するBの効果」とする概念図を制作する。ノベルティ効果は開始直後に高く、その後減衰して安定する曲線。プライマシー効果は開始直後に低く、習熟とともに上昇して安定する曲線。数値目盛りは置かず、2本の曲線名と「短期の観測」「安定後の観測」を日本語で示す。
```
...
```
# 仕上げAIへ図表制作依頼
マルチプレイヤーA/Bテストのネットワーク干渉を示す概念図を制作する。1試合の一時的なチームにA群とB群のプレイヤーが混在し、B群の新機能からA群の味方と対戦相手へ影響の矢印が伸びる。試合終了後にネットワークが解散し、次の試合で別メンバーと再形成される2段構成。「直接効果」「味方への波及」「対戦相手への波及」「試合ごとに再編成」の4ラベルだけを使う。特定ゲームを連想させるロゴや固有キャラクターは入れない。
```
...
```
# 仕上げAIへ図表制作依頼
独立した検定を各5%で行い、すべてに真の差がないと仮定したとき、少なくとも1件が偶然有意になる確率を棒グラフにする。検定数1、5、10、20に対し、FWERは5.0%、22.6%、40.1%、64.2%。横軸は「検定数」、縦軸は「少なくとも1件の偽陽性が出る確率（%）」とし、値を変更・補完しない。「独立を仮定した説明用の計算」と注記する。
```
...
[8]: https://doi.org/10.2307/4615733
