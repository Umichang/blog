...
```
# 仕上げAIへ図表制作依頼
多段階通貨の交換フローを示す概念図（数値付きフロー図）を依頼する。

構成：現金 → 創世結晶（有償通貨） → 原石（無償通貨と合算） → 紡がれた運命／出会いの縁（ガチャ1回分のトークン） → ガチャ（ウィッシュ）1回、という4段階の交換経路を示す。

数値と出典：
- 創世結晶1個＝原石1個で交換可能（一方向のみ。原石から創世結晶には戻せない）。出典：国内ゲーム攻略サイト複数（神ゲー攻略、Game8等）の課金解説記事。
- 原石160個＝紡がれた運命／出会いの縁1個（ガチャ1回分）。出典：米FTC申立書（Cognosphere/HoYoverse事案、2025年1月17日提出、Case No. 2:25-cv-00447）46項の "exchange rate of 160 Primogems for one Wish token" との記載。国内攻略サイトの換算情報とも一致。

狙い：現金支出から抽選1回に至るまで複数の交換ステップが挟まることで、実効コストが見えにくくなるという本文の主張を視覚的に補強する。数値グラフではなく概念フロー図でよい。集計時点によるずれはない（交換レートは固定値）。
```
...
```
# 仕上げAIへ画像引用依頼
米連邦取引委員会（FTC）がCognosphere／HoYoverse事案で2025年1月17日に提出した申立書（Complaint, Case No. 2:25-cv-00447, C.D. Cal.）添付の図版から、次の2点を左右比較で引用することを依頼する。

- Figure 12「'Klee' Wish Reward Revealed from Silhouette」（申立書23ページ）：実際のゲーム内演出で、星5報酬に金色の流星が伴って明かされる場面。
- Figure 13「Screenshot of Loot Box Prize Revealed in SSSniperWolf Video」（申立書24ページ）：有償プロモーション動画内で、星5報酬とされる場面の直前に紫色の流星が映っていたとされる場面。

引用の位置づけ：この2点は連邦地裁に提出された申立書に添付された図版であり、公的な法定記録の一部として提出されたものである。本節の論旨（実装にない演出上の相違が申立ての核心的事実だったこと）を裏付ける一次資料そのものであるため、著作権法上の引用要件（必要な範囲・主従関係・出典明記・無改変）を満たす引用対象として扱ってよい。出典表記には申立書のFigure番号とページ番号を明記すること。

補助候補（余裕があれば）：Figure 9「Example Event Banner Advertising "Probability increased!"」（申立書20ページ、期間限定ピックアップ画面の実例）も、本節冒頭のEvent Banner説明を補強する画像として使える。

画像が申立書PDFから鮮明に抽出できない、かつ代替の公式報道内転載画像も入手できない場合に限り、この引用は見送ってよい。
```
...
```
# 仕上げAIへ図表制作依頼
PEGIのInteractive Risk Categories（2026年3月12日発表）のうち、本稿で扱う4つの機能カテゴリーと最低年齢区分の対応を一覧できる早見図（表形式のインフォグラフィックでよい）を依頼する。

数値と出典：
- 有料ランダムアイテムを含む → 最低PEGI16（場合によりPEGI18）
- 時間または数量を限定したゲーム内オファー → PEGI12
- 毎日のクエストなど再訪を報いる「play-by-appointment」機能 → PEGI7
- 再訪しないことでコンテンツや進行を失わせる設計 → PEGI12
出典：PEGI公式発表ページ（2026年3月12日付「PEGI expands age rating criteria with interactive risk categories」）。同ページは直接取得できずJS描画のため、内容は複数の法律事務所解説記事（Reed Smith、RPC等）で照合済み。

適用範囲の注記：2026年6月以降に新規提出されるゲームが対象。既存タイトルは内容変更がない限り再格付けの対象外である旨を、図中に必ず注記すること。
```
