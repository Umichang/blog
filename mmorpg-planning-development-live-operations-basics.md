...
```
# 仕上げAIへ図表制作依頼
MMORPGの企画対象を「発売時のゲーム」だけでなく、「開発・ローンチ・長期運営」が循環するサービスとして示す概念図を作成してください。
中央に「プレイヤーが住み続けるサービス」を置き、周囲に「遊びと成長」「サーバーと決済」「経済と不正対策」「更新と復帰導線」「コミュニティとサポート」を配置してください。
開発→ローンチ→観測→更新→復帰・定住→次の観測、という循環が一目で分かる構成にしてください。数値グラフではありません。
```
