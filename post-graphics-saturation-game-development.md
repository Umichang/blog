...
```text
# 仕上げAIへ図表制作依頼
グラフィックス投資の「限界費用逓増・知覚価値逓減」を示す概念図を作成してほしい。
- 横軸：グラフィックス忠実度（フィデリティ）への投資量
- 縦軸：2本の曲線。(1) 制作コスト＝指数的に立ち上がる逓増カーブ、(2) プレイヤーが知覚する価値＝早期に飽和して頭打ちになる逓減カーブ
- 2曲線が乖離し始める領域を「飽和ゾーン（投資妙味が消える領域）」として帯で示す
- 注記：このゾーンに入った投資分を、本記事の7方向性へ再配分するのが論旨である旨を1行で添える
- スタイルはミニマルな線画。配色は本ブログの図表トーンに合わせる
```
...
```text
# 仕上げAIへ画像引用依頼
方向性②の説明の近くに、非フォトリアル表現の幅を示す参照画像を引用してほしい。
- セルシェーディング系：『Guilty Gear -STRIVE-』のインゲーム画面（3Dなのに2Dに見える例）
- 2D/3Dハイブリッド系：『スパイダーバース』または『アーケイン』のスタイルが分かるビジュアル
- ペインタリー系：『Hades II』のアートが分かるインゲーム画面
いずれも公式配布素材・プレスキットなど、出典と利用条件が明確なものに限る。キャプションに作品名・開発元・出典を明記すること。
```
...
```text
# 仕上げAIへ図表制作依頼
ライブサービスの「成否を分けるもの」を、Helldivers 2 と Concord の対比で示す図を作成してほしい。
- 2列の対比カード形式（左：Helldivers 2＝成功、右：Concord＝失敗）
- 比較軸：(1) ジャンルの飽和度、(2) 差別化の核（反応型の集団叙事 vs 既存ヒーローシューターの踏襲）、(3) 価格/参入タイミング、(4) 結果（記録的ヒット vs 早期停止・スタジオ閉鎖）
- 下部に「教訓：差別化なきライブサービス参入は、もっとも高コストな失敗になりうる」を1行で添える
- 配色は成功＝寒色系、失敗＝暖色系などコントラストを明確に
```
...
```text
# 仕上げAIへ図表制作依頼
7方向性のポジショニングマップ（2軸の散布図）を作成してほしい。
- 横軸：即効性（左＝長期で効く … 右＝短期で効く）
- 縦軸：差別化インパクト（下＝参入券・必須機能 … 上＝勝ち筋・尖った差別化）
- プロット位置の目安：
  - ①AI制作自動化＝右下寄り（即効性高・単体では差別化弱）
  - ②スタイリッシュ表現＝上・中央（差別化高）
  - ③AI NPC・動的ナラティブ＝左上（長期・差別化高だが不確実）
  - ④空間オーディオ・ハプティクス＝上・中央（差別化高・中規模でも届く）
  - ⑤ライブサービス・GaaS＝右上だが高リスク（成功時の差別化大／失敗時の損失大を注記）
  - ⑥フランチャイズIP・トランスメディア＝左上（長期・差別化高）
  - ⑦クラウド・クロスプラットフォーム＝下（参入券）
- 各点に丸番号とラベルを付す。本ブログの図表トーンに合わせたミニマルな配色で
```
...
```text
# 仕上げAIへ図表制作依頼
プロジェクト規模別の推奨注力領域マトリクスを作成してほしい。
- 行：AAA（大手）／AA（ミドルクラス）／インディー の3区分
- 列：本記事の7方向性（①〜⑦）
- 各セルを「主軸◎／補助○／優先度低△」の3段階で塗り分け、凡例を付す
- 目安：
  - AAA：①◎ ③◎ ⑤◎(高リスク注記) ⑥◎ ④○ ⑦○ ②○
  - AA：②◎ ④◎ ⑤○ ①○ ⑥△ ③△ ⑦○
  - インディー：②◎ ⑤◎ ①○ ④○ ③△ ⑥△ ⑦△
- ⑤AAAのセルには「差別化なき参入は高リスク」の小注記を付す
```
