...
社内の監視画面が赤く光って、個人情報漏洩が始まるとは限らない。発売前のゲームデータが外部サイトに現れる、利用者から身に覚えのないメールを指摘される、委託先から連絡が入る。外部で起きた異変から、社内の不正アクセスが逆算されることもある。
...
```
# 仕上げAIへ図表制作依頼
「0〜4時間」「4〜24時間」「24〜48時間」の3段階が左から右へ矢印でつながるタイムライン形式のフローチャートを作成してください。各段階には「到達させる状態」と「主な成果物」を短く要約して添えてください。
- 0〜4時間：到達させる状態＝連絡責任者と調査の入口を一本化し、証拠保全と流出遮断を開始する／主な成果物＝事案番号、事実台帳、遮断の実施記録
- 4〜24時間：到達させる状態＝データ類型、保管先、対象者群、疑われる経路を切り分ける／主な成果物＝対象データ一覧、影響範囲の概数、原因仮説と根拠
- 24〜48時間：到達させる状態＝報告トリガーを仮判定し、速報と本人通知の準備を開始する／主な成果物＝報告判定表、速報ドラフト、通知対象・連絡先の抽出条件
数値データは扱わない概念図（フローチャート）で構いません。
```
...
```
# 仕上げAIへ図表制作依頼
上記の表を俯瞰するスイムレーン形式の図を作成してください。縦軸に「事案責任者」「法務・個人情報保護担当」「開発・SRE・情報システム」「CS」「広報」「プランナー・運営担当」の6部門、横軸に「0〜4時間」「4〜24時間」「24〜48時間」の3段階を置き、各セルには表の文言を数語程度に要約したラベルを添えてください。詳細な文言は本文の表に譲るため、図はあくまで「いつ誰が動くか」の全体像をつかむための補助として、簡潔さを優先してください。数値データは扱わない概念図で構いません。
```
