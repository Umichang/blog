...
- Steam全体で実績機能が正式導入された単一の日付は、本文では断定していない。2007年9月のValve公式更新記録で『Team Fortress 2』の実績・統計が運用されていた範囲のみ記載した。
- アーケード、家庭用、PCゲームを通じた「最初の実績」の特定は行っていない。先行するハイスコア、ゲーム内称号、物理的報奨は性質が異なるため、単一の元祖として扱わなかった。
- 特定タイトルの解除率や、実績導入によるリテンション上昇率は記載していない。時期、母集団、プラットフォームによって変動し、一般化できる一次資料が必要になる。
