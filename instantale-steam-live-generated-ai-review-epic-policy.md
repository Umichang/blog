...
```
# 仕上げAIへ画像引用依頼
記事冒頭、この直後の区切り線の前に、Instantaleのプレイ画面を示す画像を1点、引用として挿入してください。
第一候補：だるまと赤べこ氏のYouTube動画「Instantaleプレイ動画（最高設定版）」（https://www.youtube.com/watch?v=b1NZCzizf7c）から、自然言語入力とAIによる動的生成の様子が伝わるフレームを1枚切り出してください。開発者本人による一次資料としてのYouTube引用は、通常は禁止ソースですが、今回は開発者本人の主要発信チャネルであることを理由にうみちゃんが承認済み・CLAUDE.mdに記録済みの例外です。
代替候補：上記が画質・構図上難しい場合は、Epic Games Store公式製品ページ（https://store.epicgames.com/p/instantale-2cfd4c）のスクリーンショットギャラリーから1枚を使用してください。
いずれの場合も、著作権法上の引用要件（必要な範囲・主従関係・出典明記・無改変）を満たす形で選定し、出典元（動画タイトル・チャンネル名、またはEpic Games Store公式ページ）をキャプションに明記してください。
```
...
```
# 仕上げAIへ図表制作依頼
自由入力型ライブ生成AIゲームを中央に置き、左へSteam、右へEpic Games Storeを配置した横長の比較図を制作してください。
Steam側は「生成時点を分類」→「ライブ生成ではガードレールを説明」→「Adult Only Sexual Contentとの組み合わせは配信不可」→「顧客への開示」という流れにしてください。
Epic側は「通常のコンテンツガイドライン」→「AIという制作手段には中立」→「開発者が説明」→「プレイヤーが判断」という流れにしてください。
Epicにも違法・ポルノ・権利侵害コンテンツの禁止があるため、「無審査」「何でも可」とは表現しないでください。
企業ロゴは使わず、文字量を抑えた日本語の概念図にしてください。
```
...
- Steamと開発者の間で交わされた審査文面、提出時のビルド、コンテンツアンケートの全回答は公開されていない。本稿の『Instantale』に関する審査経緯は、開発者が公開した動画と発表文に基づく。
- 発売発表時の国内価格は税込1,700円だったが、2026年8月14日時点のEpic Games Store国内表示は1,460円である。価格改定の日時と理由は公開資料から確認できていない。
