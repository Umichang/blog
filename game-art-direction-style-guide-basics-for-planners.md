...
では、なぜプランナーが理解する必要があるのか。それは、ゲームのルール、画面上の情報、キャラクターの役割、報酬や導線といった企画上の意図が、アートと切り離せないからである。Riot Gamesも、ゲームのアートはゲームデザインと一体に機能しなければ、ゲーム体験が混乱したものになると説明している。[[1](#ref-1)]
...
なお、本稿では「世界設定」という言葉を、キャラクターや舞台設定、技術水準など、ゲーム内の表現を支える設定のまとまりという意味で使う。「世界観」という言葉は意図的に多用しない。
...
アートディレクターの仕事には、ゲームの視覚的なアイデンティティを形作り、維持し、スタイルガイドを伝え、制作物全体の一貫性を確認することが含まれる。Ubisoftのアートディレクター職の説明でも、視覚的な基準や参考資料を設定し、アート制作をレビューして統一された品質を保つ役割が示されている。[[2](#ref-2)]
...
ここで必要なのは、ガイドを一度作って保管することではない。更新日、変更点、変更理由、承認者を残し、実際の制作で生じた例外を反映することである。AppleのHuman Interface Guidelinesも、長期にわたって更新され、変更履歴を持つ「生きた文書」として説明されている。ゲーム用の資料と同一ではないが、ガイドラインを固定された初版ではなく、製品の変化に合わせて管理する考え方の参考になる。[[3](#ref-3)]

### 目的4：IPやブランドの核を保持する

ゲームの個別アセットは増えていくが、プレイヤーがそのゲームらしさとして受け取る要素には、一定の核がある。たとえば、色の温度、形の丸さ、陰影の強さ、装飾の密度、ユーモアの出し方、キャラクターと環境の距離感などである。

アートディレクションは、見た目だけを飾る作業ではない。作品が何を大切にし、プレイヤーにどう受け取ってほしいかを、各アセットの判断へ翻訳する作業である。Ubisoftはクリエイティブディレクションについて、ゲームの見た目、物語、体験を一つの目的へ集め、方針が変化しても共通の目標を明確に保つ仕事だと説明している。[[4](#ref-4)]
...
```
# 仕上げAIへ図表制作依頼
上の表に示した構成要素（視覚的な柱、色、線と形、頭身・比率、素材感、光と陰影、領域別の方針、NG表現、参考画像、実装条件、管理情報）を、「視覚的な柱」を中心に置いたマインドマップ形式で図解してほしい。各項目から枝を伸ばし、それぞれの項目がどんな制作判断を支えるか（表の「プランナーが確認する問い」列の内容）を短い言葉で添えること。項目を暗記事項としてではなく、判断を支える地図として見せる構成にしてほしい。
```

Unreal Engineが紹介する学生ゲームの制作事例でも、冒険らしさを伝えるためにドラマチックな光と夢のような色を選び、環境や小道具は認識しやすい形に整理し、テクスチャは素材の違いを示しながら画面の読みやすさを妨げない程度に抑えている。これは一つのゲームの事例にすぎないが、スタイルを「雰囲気」という一語で済ませず、光、色、形、質感、情報量へ分解する考え方を示している。[[5](#ref-5)]

```
# 仕上げAIへ画像引用依頼
Unreal Engine公式Tech Blog「Explore En Garde! a 17th century, action-packed UE4 student game」（https://www.unrealengine.com/tech-blog/explore-engarde-seventeenth-century-action-packed-ue4-student-game）に掲載されている、環境コンセプトアートまたは室内外の照明比較画像を引用候補としてほしい。本文で言及する「ドラマチックな光と夢のような色」「認識しやすい形への整理」を視覚的に裏付ける一次資料になる。著作権法上の引用要件（必要な範囲・主従関係・出典明記・無改変）を満たす形で使用し、出典としてUnreal Engine公式Tech Blogのページと、制作チーム（Supinfogame Rubika学生チーム、En Garde!）のクレジットを明記すること。要件を満たせない場合はスキップしてよい。
```
...
```
# 仕上げAIへ図表制作依頼
「もっとかわいくしてほしい」のような曖昧な感想が、観察→影響→確認の3ステップを経て具体的な質問文へ変わる過程を、before/after形式で示す図を作成してほしい。左側に曖昧な感想、右側に変換後の質問文を置き、中央に観察・影響・確認の3ステップを矢印でつなぐ構成にすること。直後の本文で挙げる4つの実例（もっとかわいくしてほしい／リアルすぎる／既存キャラクターと合っていない／この色は違う）のうち1〜2件を具体例として反映してほしい。
```
...
<a id="ref-1"></a>1. [Game Design｜Riot Games][1] - ゲームのアートとゲームデザインを一体の体験として扱う必要性を説明している。

<a id="ref-2"></a>2. [Art Director — Art Team｜Ubisoft Careers][2] - アートディレクターが視覚的なアイデンティティ、スタイルガイド、制作基準、品質レビュー、一貫性を担うことを示している。

<a id="ref-3"></a>3. [Meet the new Human Interface Guidelines｜Apple Developer][3] - ガイドラインを更新履歴を持つ「生きた文書」として管理する考え方を示している。

<a id="ref-4"></a>4. [Creative Direction｜Ubisoft][4] - ゲームの見た目、物語、体験を共通の目的へまとめ、方針の変化に対して目標を明確に保つ考え方を説明している。

<a id="ref-5"></a>5. [Explore En Garde! a 17th century, action-packed UE4 student game｜Unreal Engine][5] - スタイル方針を光、色、形、質感、キャラクターの単純化、画面の読みやすさへ具体化した制作事例を紹介している。

[1]: https://www.riotgames.com/en/artedu/game-design
[2]: https://www.ubisoft.com/en-us/company/careers/search/744000109660515-art-director-art-team-
[3]: https://developer.apple.com/news/?id=v8a3aetj
[4]: https://www.ubisoft.com/en-us/company/careers/our-jobs/design-creative-direction/creative-direction
[5]: https://www.unrealengine.com/tech-blog/explore-engarde-seventeenth-century-action-packed-ue4-student-game
