...
```
# 仕上げAIへ図表制作依頼
本文中の6分類（ナビゲーション／操作受付／結果／例外／通知／セレモニー）を一覧できる図を作成してください。各分類ごとに役割と代表例を簡潔に添え、特に「結果」と「セレモニー」の違い（結果音は事実を短く伝える／セレモニー音は結果の見せ方を演出する）が視覚的に区別できるよう強調してください。
```
...
```
# 仕上げAIへ図表制作依頼
状態を区別するための4つの軸（音高の動き／時間／音色／リズム）を一覧できる図を作成してください。各軸について、本文で挙げている選択肢（例：音高の動きなら上昇・下降・反復・解決しない音程差）を簡潔に添えてください。
```
...
```
# 仕上げAIへ図表制作依頼
ガチャ・抽選演出の5段階（1. 抽選開始／2. 抽選処理進行中／3. 開示開始／4. レアリティ・結果確定／5. 獲得記録）を時系列のフロー図として作成してください。1・2を「期待を作る演出」、4・5を「結果を伝えるフィードバック」としてグループを視覚的に分け、通信中に結果音を鳴らさず確定後に音と表示を同期させるという注意点も図中に添えてください。
```
...
```
# 仕上げAIへ図表制作依頼
プランナーが発注仕様書を作るまでの流れを循環図として作成してください。「状態の分類」→「仕様書化（発生条件・優先度・再生頻度などの項目を整理）」→「発注」→「プレイテストでの検証」→（分類の見直しへ戻る）という一連のサイクルを示してください。
```
