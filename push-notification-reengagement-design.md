...
![通知疲れを構成する3つの要因（慣化・関連性の低下・割り込みコスト）の関係図](image/notification-fatigue-three-factors.webp)
...
![通知許可の5状態（未提示・ソフトアスク保留・OS許可済み・OS拒否済み・許可後に無効化）とその遷移を示す状態遷移図](image/notification-permission-state-machine.webp)
...
![全体キャップ・種類別キャップ・キャンペーンキャップが入れ子になっている頻度制御の構造図](image/notification-frequency-cap-nested-structure.webp)
...
![到達可能性・内容適合・時間適合・疲労の4群から関連性スコアが算出される構成図](image/notification-relevance-scoring-four-factors.webp)
...
![通知拒否後に割り込みの弱い順で案内する代替チャネルの階層図](image/notification-opt-out-fallback-channels.webp)
...
![許可から長期影響までの通知施策KPIの流れ図](image/notification-kpi-funnel.webp)
