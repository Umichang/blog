...
```
# 仕上げAIへ画像引用依頼
記事冒頭のヘッダー画像として、Mojang/Microsoft公式のプレスキットまたは公式記事内の画像を候補とする。優先順は次の通り。(1) 2017年9月20日のBetter Together Update発表時の公式キーアート、または公式のBetter Together FAQページに掲載されている画像。(2) Minecraft Live 2023の累計3億本発表を報じる公式ブログ内の画像。著作権法上の引用要件（必要な範囲にとどめる、本文に対して従たる位置づけにする、出典を明記する、無改変で使用する）を満たす画像に限定し、満たす候補が見つからない場合は挿入をスキップする旨を明記する。
```
...
開発者のMarkus Persson（Notch）は初期、強い販売がいつまでも続くとは考えておらず、販売が落ち着いた後にはソースコードを公開する意向まで示していた。[[3](#ref-3)] この逸話は、今日の持続性を「最初から完成された長寿設計」として説明することへの警戒になる。現在の『Minecraft』は、初期の自由度だけで成立しているのではない。後年に積み重ねられた、更新の運営とプラットフォーム戦略の結果でもある。
...
```
# 仕上げAIへ図表制作依頼
EChartsで横方向の1軸年表を制作する。横軸は年（2009、2014、2017、2018、2019、2020、2021、2022、2023、2024）とし、2009「Java版提供開始」、2014「MicrosoftがMojangを25億ドルで買収」、2017-09-20「Better Together」、2018-06-21「SwitchがBedrockへ」、2019-12-10「PS4がBedrockへ」、2020「Nether Update」、2021「Caves & Cliffs」、2022「Wild Update」、2023「Trails & Tales／累計3億本超」、2024「Tricky Trials」を配置する。上段は事業・版の展開を青、下段は大型アップデートを緑で示す。数値の補完や丸めは行わず、日本語のラベルは短くして重なりを避ける。白地、Hiragino系フォント、凡例あり。完成したPNGを確認してWebP化し、次の位置へ挿入する。
```
...
```
# 仕上げAIへ図表制作依頼
Java版とBedrock版の違いを整理した簡易な2カラム比較図を作成する。項目は次の3つ。「クロスプレイの相手」（Java版：Windows・Mac・LinuxのJava版のみ／Bedrock版：Windows・コンソール各種・モバイル）。「改変の入口」（Java版：Mod、有志コミュニティが独自に開発・配布／Bedrock版：Add-On、Minecraft Marketplaceで公式に流通）。「自前サーバーとRealms」（Java版：自前サーバーを立てる文化が強い／Bedrock版：Realms Plusを含む公式Realmsが入口になりやすい）。出典は本文で参照している公式比較ページ（Java or Bedrock Edition）。数値は扱わないため実数の記載は不要。左右対称の2カラムレイアウトとし、見出しに「Java版」「Bedrock版」を明記する。白地、Hiragino系フォント。
```
