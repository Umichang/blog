...
```
# 仕上げAIへ図表制作依頼
1999年から2012年までのガラケー時代モバイルゲーム文化史の主要トピックを一本の時系列で示すインフォグラフィックを作成してほしい。1999年iモード/EZweb/J-SKY開始、2002年着うた開始、2004年ケータイ小説文化隆盛とiモード都度課金導入、2006年GREE/モバゲータウン台頭、2007年ガチャ的収集要素の普及、2012年コンプガチャ問題、2008年以降のスマートフォンへの地殻変動、という流れを横軸の年表として配置する。記事全体の見取り図として冒頭に置くため、専門用語よりも出来事の流れが一目でわかることを優先してほしい。ファイル名は feature-phone-history-timeline.webp とする。
```
...
```
# 仕上げAIへ図表制作依頼
携帯電話キャリアの公式サイトモデルの構造を示す図を作成してほしい。キャリアを中心に置き、審査を経て公式メニューへ掲載されるコンテンツ事業者と、URLを直接入力してアクセスする「勝手サイト」を対比しつつ、公式サイト側では「審査→メニュー掲載→利用者の発見→利用登録→キャリアによる情報料徴収代行→通信料金と合算した請求」という一連の流れを矢印でつなげて示す。現在のアプリストアとの対比の土台となる図なので、後の段落で「現在のアプリストアに似ている」と述べている点が視覚的にも伝わる構成にしてほしい。ファイル名は carrier-official-site-ecosystem.webp とする。
```
...
```
# 仕上げAIへ画像引用依頼
『不思議のダンジョン 風来のシレンMEGA2』（携帯電話向け、2007年）の操作画面のスクリーンショットを引用候補として探してほしい。数字キーを8方向移動に、中央の5キーを攻撃・決定に割り当てた操作レイアウトが視覚的にわかる画面が望ましい。著作権法上の引用要件（必要な範囲・主従関係・出典明記・無改変）を満たす画像が見つからない場合は、その旨を明記して挿入をスキップしてほしい。
```
...
```
# 仕上げAIへ図表制作依頼
フィーチャーフォンの技術的制約（10KBのアプリ容量、方向キー・決定キー・数字キーという入力方式、遅い回線とパケット通信料、頻繁な中断）が、どのような設計原則（一画面一判断の構造、片手操作前提のシンプルな操作系、生活の隙間に戻れる短い遊びの単位）へ変換されたかを、左に制約、右に設計原則を並べて矢印で対応関係を示す図を作成してほしい。ファイル名は feature-phone-constraint-to-design.webp とする。
```
...
```
# 仕上げAIへ画像引用依頼
2006年前後のモバゲータウンまたはGREEにおける、アバター（髪型・服・アクセサリー等の組み合わせ）をプロフィールに表示している当時の画面のスクリーンショットを引用候補として探してほしい。著作権法上の引用要件（必要な範囲・主従関係・出典明記・無改変）を満たす画像が見つからない場合は、その旨を明記して挿入をスキップしてほしい。
```
...
```
# 仕上げAIへ図表制作依頼
ガチャという収益モデルが単独の発明ではなく複数の要素の合流点であったことを示す図を作成してほしい。「携帯電話における少額決済の習慣（着メロ・着うた・都度課金）」「SNSにおける収集物を見せ合う文化（アバター・ランキング）」「運営型ゲームにおける継続的な新商品追加の仕組み」という3つの流れが1本の矢印に合流し、その先に「ガチャ」という収益装置が生まれる様子を示す構成にしてほしい。ファイル名は gacha-model-convergence.webp とする。
```
...
```
# 仕上げAIへ図表制作依頼
「現代のプランナーが持ち帰る四つの教訓」の4項目（制約は核を露出させる／決済摩擦の低下は商品設計まで変える／プラットフォーム依存は成功するほど抜けにくくなる／射幸心を強く刺激する設計は短期収益ほど後で高くつく）を一覧できるまとめインフォグラフィックを作成してほしい。各項目を簡潔な一言のラベルと共に4つのブロックまたは4象限で配置し、記事を読み終えた読者が要点を持ち帰れる構成にしてほしい。ファイル名は feature-phone-four-lessons-summary.webp とする。
```
