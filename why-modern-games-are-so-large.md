...
実例として、『FINAL FANTASY VII REBIRTH』の開発陣はPlayStation公式インタビューで、ディスク容量を開発初期から要件に置き、前作で使ったOodle KrakenとOodle Textureの経験を容量計画へ生かしたと説明している。これは、圧縮技術そのものだけでなく、 **早期に容量を制約として扱うこと** が重要だと分かる一次情報である。[[14](#ref-14)]
...
- 本文では、パッチで変動しやすい特定タイトルの現在のインストール容量を記載していない。
- 『FINAL FANTASY VII REBIRTH』については、公式インタビュー内の開発時のディスク容量要件や前作容量にも具体値があるが、本稿の論旨に不要なため本文では数値を省いた。将来追記する場合は、パッケージ版・ダウンロード版・地域・更新時点を分けて再確認すること。
- 旧世代の光学メディアやHDDで高速アクセスのため同一データを近接配置した個別事例は、タイトルごとの一次資料を確認できていないため、一般的な設計上の選択肢としてのみ記述した。
...
<a id="ref-14"></a>14. [How Final Fantasy VII Rebirth harnesses immersive PS5 technology][14] - 開発初期からディスク容量を要件化し、Oodle系圧縮技術の経験を容量計画へ使ったと開発者が説明するPlayStation公式インタビュー。
