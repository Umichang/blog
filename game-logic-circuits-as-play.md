...
```
# 仕上げAIへ図表制作依頼
AND・OR・NOTを、ボタン・扉・スイッチ・明かりで説明する概念比較図。背景は白。上からAND、OR、NOTの順に独立した段を置く。
ANDの段は「両方オンで開く」とし、Aオン・Bオフでは扉が閉じ、Aオン・Bオンでは開く例を示す。
ORの段は「片方でもオンなら開く」とし、Aオン・Bオフで扉が開き、Aオン・Bオンでも開く例を示す。
NOTの段は「オン・オフを反対に」とし、消灯スイッチがオンなら明かりが消え、オフならつく例を示す。
図中の語句とaltは日本語。AND・OR・NOTの名称だけは英字とする。オンとオフは文字と絵の状態で区別し、色だけに依存しない。回路図の記号、電気の専門用語、ゲーム固有の設備画像は使わない。数値は不要であり、推測で追加しない。
```
