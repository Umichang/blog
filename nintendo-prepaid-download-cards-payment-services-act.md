...
```
# 仕上げAIへ図表制作依頼
コンビニに並ぶ2種類のカードを左右対比する概念図を作る。左は「プリペイドカード」から「残高」へ進み、その後に複数のソフトから選ぶ流れ。右は「ダウンロードカード」から「決められた1本」へ直接進む流れ。中央に「後から選べるか」を短く置く。カードの商品画像や企業ロゴは使わず、一般化したカード形状とゲーム画面のアイコンで表現する。数値グラフではない。
```
...
```
# 仕上げAIへ図表制作依頼
前払式支払手段の有効期限設定を起点とする、シンプルな二分岐の概念図を作る。
起点：「前払式支払手段の有効期限をどう設定するか」
分岐条件：「発行日から6か月以内に限られるか」
「はい」の枝：「資金決済法の適用除外（届出・供託・保全契約のいずれも不要）」
「いいえ」の枝：「前払式支払手段として規制対象（基準日ごとに未使用残高を集計し、供託・保全契約・信託契約のいずれかで2分の1以上を保全）」
特定の企業名・商品名・ロゴは使わず、一般化した分岐図（フローチャート）として表現する。数値グラフではない。
```
