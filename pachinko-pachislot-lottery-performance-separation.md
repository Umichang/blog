...
```
# 仕上げAIへ図表制作依頼
主基板と演出制御側の一方向通信構造を示す概念図を制作する。
左に「限定された遊技入力」として、パチンコの入賞センサー、パチスロのメダル投入・開始レバー・停止ボタンを小さく配置する。中央に「主基板（抽選・払出・リール制御）」、右に「演出制御側（液晶・サウンド・ランプ・可動役物）」を配置する。
入力から主基板へは細い矢印、主基板から演出制御側へは太い一方向矢印で「抽選結果・変動時間などのコマンド」と記す。演出制御側から主基板へ向かう方向には矢印を描かず、遮断記号と「受信経路なし」を明示する。
「演出は結果を変更できない」が一目で分かる構成とする。数値グラフではない。日本語ラベルは上記の短い語句に限定し、16:9、明るい不透明背景、記事幅で読みやすい設計にする。図中にFig番号は入れない。
```
...
```
# 仕上げAIへ図表制作依頼
遊技機開発における3層プログラム構造の概念図を制作する。
左から右へ「メインプログラム」「サブメイン（演出制御）」「サブサブ（液晶制御）」の3列を並べる。
メインには「当落・役の抽選」「払出」「リール制御」、サブメインには「コマンド解析」「演出パターン選択」「音・ランプ・役物の統合」、サブサブには「映像表示」「UI・アイコン」「素材再生」と記す。
矢印はメイン→サブメイン→サブサブの一方向だけにする。メインとサブメインの境界を太い線で強調し、「遊技結果の境界」と短く添える。
数値グラフではない。日本語ラベルの正確さを優先し、16:9、明るい不透明背景、記事幅で読める文字量にする。図中にFig番号は入れない。
```
