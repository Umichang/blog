...
```
# 仕上げAIへ図表制作依頼
『塊魂』シリーズの主要リリースを時系列で示す年表チャート（横棒または年表形式）を作成してください。
データと出典は次のとおりです。
- 2004年3月18日 塊魂（PS2）／2005年 みんな大好き塊魂（PS2）／2005年 僕の私の塊魂（PSP）：出典は株式会社ナムコ「『塊魂』シリーズ参考資料」（2005年10月3日付、記事References [1]）
- 2007年 ビューティフル塊魂（Xbox 360）／2009年 塊魂TRIBUTE（PS3）／2011年 塊魂 ノ・ビ〜タ（PS Vita）／2018年 塊魂アンコール（Nintendo Switch・PC）／2020年 塊魂アンコール（PS4・Xbox One）／2025年4月 塊魂 Rolling LIVE（Apple Arcade）：出典は電ファミニコゲーマーの記事（記事References [3]）
月日まで確定しているのは2004年3月18日と2025年4月3日のみです。それ以外は年単位の情報として扱ってください。非開示・未確認の月日は空欄のままにしてください。
```

```
# 仕上げAIへ画像引用依頼
初代『塊魂』（2004年、ナムコ、PlayStation 2）のプレイ画面で、王子が塊を転がしている様子が確認できる画像を、著作権法上の引用要件（必要な範囲・主従関係・出典明記・無改変）を満たす形で使えるか検討してください。バンダイナムコエンターテインメントの公式サイトや公式資料に掲載されている画像を優先して探し、要件を満たす候補が見つからない場合は掲載を見送ってください。
```
...
```
# 仕上げAIへ図表制作依頼
第4章の内容に対応する概念図を作成してください。左側に没になったドライビングゲーム案（人を殴って気絶させる設定を持つハンマー型の頭のキャラクター、暴力的なカージャックの世界設定）、右側に『塊魂』の王子（同じハンマー型の頭のシルエットのみを残し、暴力的な設定や乗り物操作の要素は取り除かれている）を対比させ、何を残し何を捨てたかが視覚的にわかる図にしてください。
```
...
```
# 仕上げAIへ図表制作依頼
第5章の内容に対応する概念図を作成してください。「転がす」という単一の入力操作を中心に置き、そこから移動・収集・成長・探索・障害の解消・達成感という6つの体験が分岐して生まれる構造を示してください。単一ルールが要素の少なさではなく、体験の多重化を生んでいることが伝わる図にしてください。数値要素は不要です。
```
