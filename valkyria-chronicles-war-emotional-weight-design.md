---
description: "『戦場のヴァルキュリア』の水彩表現、BLiTZ、パーマデス、架空欧州は、戦争の重みをプレイヤー自身に引き受けさせる一つの設計目標から生まれた。"
---
...
```
# 仕上げAIへ図表制作依頼
非数値の概念図。初期試作と完成版を左右に対比させる。破棄された初期試作の実画面は現存しないと見られるため、実際のゲーム画面を模したものではなく、模式図として作成すること。
左側「初期試作（破棄）」：見下ろし視点、盤面上の駒（例えば「戦車5両」とまとめて表示される一つの駒）、グリッドは使わないが俯瞰のみの構図。
右側「完成版（BLiTZ）」：三人称視点で一人の兵士の背後に立つ構図、名前と個性を持つ個別キャラクター。
両者の間に「個々の喪失が見えるか」という対比軸を示す短い矢印またはラベルを置く。戦争の生々しい描写や流血表現は避け、鉛筆線と淡い水彩を思わせる落ち着いた配色にする。日本語文字は指定文言のみ正確に配置する。
```
...
```
# 仕上げAIへ画像引用依頼
セガ公式の初代PS3版『戦場のヴァルキュリア』サイトから、BLiTZのコマンドモードとアクションモードの違いが本文の説明に必要な範囲で分かるゲーム画面を探す。引用要件を満たせる場合は、モードごとに1枚ずつ、画面を改変・合成せずWebPへ変換して配置し、日本語altと「画像出典（引用）：セガ公式サイト」の出典注記を付ける。適切な画像が見つからない場合は、このブロックだけを完全に削除する。
候補URL：https://portal.valkyria.jp/vc1/ps3/
```
...
```
# 仕上げAIへ図表制作依頼
非数値の概念図。横長1枚。中央に「戦争の重みをプレイヤー自身に引き受けさせる」を置き、周囲の4要素から中央へ矢印を集める。4要素は「BLiTZ：指揮官視点と兵士視点を往復」「CANVAS：写実性を下げ、感情を近づける」「兵科一括成長＋パーマデス：能力差を抑え、人物の喪失を残す」「架空欧州＋ウェルキン：歴史の複雑さを絞り、争いを乗り越える問いを残す」。各要素の下に小さく「遠ざけたもの→近づけたもの」を示す。戦争写真のような写実表現や流血は使わず、鉛筆線と淡い水彩を思わせる落ち着いた概念図にする。本文の固有キャラクターを描写せず、抽象的な指揮図、人物シルエット、関係線、本のモチーフで構成する。日本語文字は指定文言だけを正確に配置する。
```
