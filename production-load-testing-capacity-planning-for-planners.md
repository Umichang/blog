...
```
# 仕上げAIへ図表制作依頼
DAU・ログイン到着率・CCU・RPS/TPSなど、負荷モデルに使う指標同士の関係を示す図を制作する。DAU・新規登録数を起点に、そこから「ログイン到着率」「CCU」を経て「RPS・TPS」「イベント到着率」へとつながる変換の流れを矢印で示す。各指標の定義は本文の表と一致させ、「ログイン到着率×平均セッション時間→CCU」のような簡潔な変換式を矢印に添える。技術用語を並べるだけの図にせず、指標が積み上がっていく構造が一目でわかるようにする。図中にFig.nやFigure nの番号は入れない。
```
...
```
# 仕上げAIへ図表制作依頼
ゲーム特有のボトルネックとして本文が挙げる「ログイン認証」「マッチメイキング」「データベース書き込み」「ガチャ・報酬・インベントリ」「決済」の5領域を横並びまたは放射状に配置した図を制作する。各領域の下に、本文で挙げている確認項目を1〜2個、短い日本語で添える(例:ログイン認証なら「トークン発行」「セッション作成」、決済なら「レシート検証」「再送」など)。5領域が独立したチェック項目ではなく、いずれもユーザー体験の途中経路として連鎖しうる点が伝わるよう、緩やかな接続を示す線を添えてもよい。図中にFig.nやFigure nの番号は入れない。
```
...
```
# 仕上げAIへ図表制作依頼
本番想定の負荷試験とキャパシティプランニングの流れを、ゲームプランナーにも読める横長の工程図として制作する。左から「事業情報の収集」「トラフィックプロファイル作成」「本番相当環境」「ベースライン・ロード・スパイク・ソーク試験」「ボトルネック特定」「スケーリングと劣化設計」「リリース判定・当日運用」の順に並べる。各工程の下に、入力と成果物を短い日本語で示す。技術用語はCCU、RPS、SLO、キューに限定し、文字量を少なくする。矢印は一方向だけでなく、試験結果からモデルへ戻る改善ループも表現する。図中にFig.nやFigure nの番号は入れない。
```
...
```
# 仕上げAIへ画像引用依頼
Microsoft Azure Architecture CenterのQueue-Based Load Leveling Patternの公式ページに掲載されているアーキテクチャ図(到着するリクエストとキュー、キューから処理するワーカーの関係を示す図)を引用候補として検討する。著作権法上の引用要件(必要な範囲・主従関係・出典明記・無改変)を満たせる場合に採用し、出典として当該ページのURLを明記する。要件を満たす形で引用できない場合は、この依頼をスキップしてよい。
```
...
### FINAL FANTASY XIV: Endwalker：ログインだけでなく移動集中を見る

FINAL FANTASY XIV: Endwalkerのアーリーアクセスについて、公式説明は過去最大の同時接続、ワールドごとのログイン上限、エリア移動の同時集中によるネットワーク負荷を挙げている。対応として、同時にエリア移動できるキャラクター数を調整し、瞬間的なネットワーク負荷を制限したことも説明されている。[[10](#ref-10)]
...
後年のDawntrailの告知では、Endwalker時の混雑を踏まえ、データセンターの拡張や新ワールドの追加、ログインキュー、データセンタートラベルなどの制限が案内された。[[11](#ref-11)] ここで注目すべきなのは、容量を増やす対策と、入場を制御する対策が併記されている点である。負荷試験の結果を、インフラ増強だけでなく、上限、キュー、機能制限の設計へ戻すことが重要である。
...
<a id="ref-10"></a>10. [Endwalker Congestion and World Status in Early Access - FINAL FANTASY XIV: The Lodestone][10] - Endwalker開始時の同時接続、ログイン上限、エリア移動集中、負荷制限に関する公式告知。

[10]: https://na.finalfantasyxiv.com/lodestone/news/detail/2d402ccbc4eb78427223ea9420fa3e81cf20dccf

<a id="ref-11"></a>11. [Regarding Congestion During Dawntrail's Launch - FINAL FANTASY XIV: The Lodestone][11] - 過去の混雑を踏まえたデータセンター拡張、新ワールド、ログインキュー、機能制限の告知。
