...
```
# 仕上げAIへ図表制作依頼
第1章の内容に対応する概念図を作成してください。伏せ（ホフク）・しゃがみ・立ちという3つの姿勢によって、カムフラージュ率が「高い」「普通」「低い」の3段階でどう変わるかを示してください。出典はコナミ公式オンラインマニュアル「カムフラージュ」（記事References [3]）。数値の厳密なパーセンテージは公開されていないため、3段階の相対的な高低関係のみを表現し、具体的な数字は書き入れないでください。
```
...
```
# 仕上げAIへ図表制作依頼
第3章の内容に対応する分岐図を作成してください。「拘束」を起点として、そこから「尋問」「盾」「投げ」「気絶（首絞め）」「殺害（喉斬り・首折り）」の5つへ分岐する構造を示してください。出典はコナミ公式オンラインマニュアル「基本操作」（記事References [8]）。
```
...
```
# 仕上げAIへ画像引用依頼
オリジナル版『METAL GEAR SOLID 3 SNAKE EATER』のジャングル潜入シーン、またはリメイク版『METAL GEAR SOLID Δ: SNAKE EATER』での同シーンの再現画像を、著作権法上の引用要件（必要な範囲・主従関係・出典明記・無改変）を満たす形で使えるか検討してください。コナミの公式サイトや公式資料に掲載されている画像を優先して探し、要件を満たす候補が見つからない場合は掲載を見送ってください。
```
...
```
# 仕上げAIへ図表制作依頼
第5章の内容に対応する対応図を作成してください。本文中の表（更新した要素／そのままでは起きる体験の変化／補償的な調整／守ろうとしたもの）を、視覚的な対応関係図として表現してください。「写実的なモデル」と「NEW STYLEの三人称視点」という2つの更新要素それぞれから、それが引き起こす変化と、それを相殺する補償的な調整が矢印でつながる構造にしてください。
```
