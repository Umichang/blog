...
eスポーツは、「ゲームがうまい人へ高額賞金を出すイベント」ではない。
...
そして1990年代末から2000年代初頭の韓国では、『StarCraft』、PC bang、ゲーム専門ケーブルチャンネル、プロリーグが結びついた。 **PC bang** は、ネットワークゲームを遊べる時間貸しのPC店舗である。研究では、初期の韓国eスポーツを、PC通信、『StarCraft』とPC bang、放送と観戦の制度化という段階で捉えている。[[4](#ref-4)] ここで重要なのは、単発大会の規模ではない。定期的に試合を制作し、実況・解説を付け、スター選手とチームを継続して見せる、 **放送コンテンツとしての競技** が成立したことだ。
...
```
# 仕上げAIへ図表制作依頼
以下の4つの資金調達モデルが賞金プールへ合流する様子を、簡易フロー図（複数の入力元→1つの賞金プールへ矢印で合流するイメージ）で表現してください。

入力元（4モデル）：
- パブリッシャー直接拠出（宣伝・競技育成費として賞金を用意）
- スポンサー拠出（露出・企画・命名権などの対価）
- ゲーム内購入連動（対象アイテム売上の一部を賞金へ加算）
- 参加料・地域コミュニティ（参加料や地域スポンサーで運営費・賞金を組み立てる）

出力先：
- 1つの「大会賞金プール」

デザイン上の要望：
- 実際の大会は複数のモデルを組み合わせた「混合型」になりやすいことが伝わるよう、4つの入力元から賞金プールへ複数の矢印が同時に合流するイメージとする
- 各モデルの強み・リスクまでは図に含めず、直前の表を補完する位置づけのシンプルな概念図とする
- 記事本文の表デザインと視覚的に対立しないよう、控えめな配色とする
```
...
配信はテレビの代用品ではない。チャット、投票、ドロップ、複数視点、巻き戻し、共同視聴により、観客を参加者へ近づけた。人気配信者による **コストリーミング** 、つまり公式映像を許諾の範囲で同時視聴・解説する形式は、既存コミュニティへ大会を届けられる。
...
```
# 仕上げAIへ図表制作依頼
以下の6つのステップを、時計回りの循環図（円環図）として表現してください。各ステップは矢印で次のステップへつながり、最後のステップ（6）から最初のステップ（1）へ矢印で戻ることで、循環構造であることを示してください。
...
デザイン上の要望：
- シンプルな円環（サイクル）図とし、各ステップは短いラベルで表示する
- 「どこか一つだけ太くても、循環は続かない」という直後の本文の主張と対応するよう、6つの要素は均等な大きさ・重みで配置し、特定のステップを強調しない
- 色数は控えめにし、記事全体のトーンを損なわない落ち着いた配色とする
```
...
<a id="ref-4"></a>4. [Historiography of Korean Esports: Perspectives on Spectatorship][4] - 韓国初期eスポーツをPC通信、PC bang、『StarCraft』、放送と観戦の制度化から分析した研究。
