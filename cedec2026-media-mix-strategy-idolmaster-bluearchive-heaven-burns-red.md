# CEDEC2026予習：国内ゲームオリジナルIPのメディアミックス展開に見る企画論点――『アイドルマスター』『ブルーアーカイブ』、そして『ヘブンバーンズレッド』
...
```
# 仕上げAIへ図表制作依頼
本稿で扱う三事例（『アイドルマスター』『ブルーアーカイブ』『ヘブンバーンズレッド』）の全体俯瞰図を作成してほしい。第2章の比較表（IPの核×相性のよい手法）に沿って、横軸に「キャラクター性が核」から「物語性・世界設定が核」までのグラデーション、縦軸に「王道展開（ライブ・グッズ等）」から「実験的展開（ASMR・舞台等）」までのグラデーションを取ったマッピング図とし、三作品をそれぞれの位置にプロットする形式が望ましい。読者が記事全体の構造を先に掴めるようにするための図であるため、細部の演出よりも一目で三者の立ち位置の違いが伝わることを優先してほしい。
```
...
```
# 仕上げAIへ図表制作依頼
『ブルーアーカイブ』の「日常への浸透」を示す接点マップを作成してほしい。IPコア（作品名・キャラクター）を中心に配置し、そこから放射状にコラボカフェ（アニメイトカフェ等）、コンビニエンスストア（ローソン一番くじ等）、カラオケ（JOYSOUND）、遊園地・動物園（東武動物公園等）、街頭タイアップ（秋葉原電気街まつり等）の各接点を線でつなぐ図。各接点が「日常のどの行動に接続しているか」を短い添え書きで示すと、第4章の論旨（生活圏に接点を増やす展開）が視覚的に伝わりやすい。
```
...
```
# 仕上げAIへ図表制作依頼
ASMRコンテンツ商品化の先行タイムライン図を作成してほしい。2022年4月（『ブルーアーカイブ』「ノノミASMR」商品化）→2024年（ASMRイヤホン第1弾「YUUKA Ver.」）→2025年7月（ASMRイヤホン第2弾「HINA Ver.」）→2026年7月CEDEC2026（『ヘブンバーンズレッド』のメディアミックスセッション）の4点を時系列で並べる図。本章の主張（ASMR活用はヘブンバーンズレッド固有の手法ではなく、業界で先行例があった）を読者が一目で理解できるようにすることが目的であるため、年代の間隔を誇張してでも「先行→後続」の関係が明確に伝わる構成にしてほしい。
```
...
```
# 仕上げAIへ画像引用依頼
舞台『ヘブンバーンズレッド』の公式キービジュアルを引用候補として検討してほしい。公式サイト（officeendless.com/sp/hbr_stage/）または公式Xアカウント（@heavenburnsred、@hbr_stage）に掲載されているキービジュアルを想定。引用の際は、著作権法上の引用要件（必要な範囲・主従関係・出典明記・無改変）を満たすことを確認し、満たせない場合はスキップしてよい。
```
...
```
# 仕上げAIへ図表制作依頼
第1章で示した「守るもの」「広げられるもの」「検証するもの」の3ステップと、第8章の評価フレームワーク（形を変えても壊れない要素の分解／ファンの期待とのズレが生まれやすい箇所／企画を通した後の品質維持）を接続したフローチャートを作成してほしい。企画書や監修会議のチェック項目として実務的に使える体裁が望ましく、各ステップに第8章で挙げた具体的な確認観点を短く添えるとよい。
```
