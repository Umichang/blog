...
```
# 仕上げAIへ図表制作依頼
政治システムをゲームへ翻訳する流れを示す横長の概念図を制作する。左から「抽象テーマ（政治・外交）」→「中核プロセス（反政府運動／クーデター／フィンランド化／危機）」→「政策動詞（支援する／介入する／条約を結ぶ／撤回を迫る／譲歩する）」→「反応と状態変化（威信／陣営／危機水準）」→「次の年の再選択」と配置し、最後から政策動詞へ戻る反復矢印を付ける。冷戦の赤青対立を想起させつつ、国旗や実在政治家の肖像は使わない。日本語の文字量を抑え、記事幅で読み取れる構成にする。
```
