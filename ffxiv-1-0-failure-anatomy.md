...
```
# 仕上げAIへ画像引用依頼
対象：GDC 2014 吉田直樹氏の講演「Behind the Realm Reborn」で示された、旧版の植木鉢（約1,000ポリゴン・約150行のシェーダー）のスライド画像。
用途：グラフィックス偏重の象徴として本文を補強する。
出典明記：講演者・講演名・GDC 2014・出典URL（参照[7]のGDC Vault）を必ず併記する。
備考：引用可否（ライセンス・権利処理）は仕上げAIの判断に委ねる。引用が難しい場合は、装飾オブジェクト1個がプレイヤーキャラクター1体分の負荷に相当することを示すオリジナルの比較概念図で代替してよい。
```
...
# 仕上げAIへ図表制作依頼
種類：数式・概念ビジュアル
主題：UI性能がプレイ体験全体に与える影響の掛け算
要件：
- 「1操作の時間 × 1セッションの反復回数 × 影響するプレイヤー数 ＝ 体感上の総コスト」という掛け算を視覚的に示す。
- 各因子に簡潔な例（例：1操作＝数秒、反復＝1セッションに数十回、プレイヤー＝多数）を添える。
- 「1回は小さくても、積み重なるとプレイ時間の大部分を占める」というメッセージが伝わる構成にする。
- 日本語ラベル。
```
...
# 仕上げAIへ図表制作依頼
種類：階層ピラミッド図（4層）
主題：オンラインゲームで「発売後に修正できる層」の難易度階層
要件：
- 上から順に「数値（報酬量・敵HP・ドロップ率）」「コンテンツ（クエスト・ダンジョン・装備）」「体験設計（バトル・成長・経済・導線）」「基盤（サーバー構成・クライアント・エンジン・データ構造）」の4層を積む。
- 上層ほど「発売後の修正が容易」、下層ほど「稼働中の交換が困難」であることを、色の濃淡または幅で表現する。
- 「旧版は下二層に問題を抱えたまま、上二層のパッチで回復できるように扱った」という論点を補足する注記を図中または図脇に添える。
- 日本語ラベル。
```
...
```
# 仕上げAIへ画像引用依頼
対象：旧版（1.0）と『新生エオルゼア』のマップまたはUIを並べた比較スクリーンショット。
用途：4章「対比として見る」を視覚的に補強し、刷新の規模を示す。
出典明記：スクウェア・エニックス『ファイナルファンタジーXIV』の権利表記と、各画像の出典を必ず併記する。
備考：引用可否（ライセンス・権利処理）は仕上げAIの判断に委ねる。公式素材の引用が難しい場合は、当該箇所への画像挿入を見送ってよい。
```
...
# 仕上げAIへ図表制作依頼
種類：概念図（相互拘束・連鎖構造図）
主題：大型タイトル開発における「判断の連鎖」と部分最適の罠
要件：
- 「組織」「技術」「UI」「コンテンツ」「運営」の5要素をノードとして配置し、相互に矢印で結ぶ。
- 一方の判断の遅れが他方の前提となり、互いを拘束し合う関係を示す。
- 起点に「ある部署にとって合理的な一つの判断（遅れ）」、終点に「全体を壊さなければ直せない状態」を置き、連鎖の流れを表現する。
- 「各部署にとっては合理的な判断が、全体では互いの逃げ道を塞ぐ」という本文の主張が一目で伝わる構成にする。
- 日本語ラベル。配色は落ち着いたトーンとし、警告色（赤系）は連鎖の最終地点のみに用いる。
```
