---
description: "「名前は6文字まで」の裏にある、数え方・使える文字・表記の揃え方を解説。ドラクエ、絵文字、フォント、NGワード判定を題材に、名前入力の仕様で決めることを整理する。"
...
```
# 仕上げAIへ図表制作依頼
文字の数え方の違いを示す概念図。上段に「👩🏽‍🚒」と「見た目に近いまとまり：1」、下段に「女性：2」「肌の色：2」「結合文字：1」「消防車：2」の四つの箱を置き、「C#のLength：7」と結ぶ。数値はMicrosoft LearnのReferences [3]による。結合文字はU+200Dであり、女性と消防車を単に並べた表現と混同しないこと。絵文字を一体表示できる環境での例である旨を添える。文字コードの番号一覧やバイト数は加えない。背景は白。ラベルとaltは日本語。数値を推測で補わない。
```
