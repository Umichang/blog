...
個別タイトルについて「AI車が何％速くなる」などの説明をするには、公式資料、コード解析、再現可能な検証の区別が要る。本記事では確認できない固有値を挙げない。
...
DDAは裏で動くため、負けた理由を説明する物語としても使われやすい。特許がある、似た挙動を見た、動画で再現した、という事実は、それぞれ証拠の強さが違う。

| 区分 | 何が確認できているか | 本文での扱い |
|---|---|---|
| (a) 開発者・公式が明言 | 目的、入力、変更対象などを公式資料で説明 | 確認された範囲を事実として書く |
| (b) 解析者が発見 | コード、メモリ、データ、反復試験から挙動を提示 | 解析結果と明記し、版や条件を限定する |
| (c) プレイヤーの考察・噂 | 体感、少数の動画、出典不明の転載 | 事実として書かない |

本記事で扱った『Left 4 Dead』は(a)だが、公式が説明したのは適応的なペース制御であり、敵性能を能力へ合わせるDDAとは区別した。『バイオハザード RE:4』（2023年版）の難易度変更も(a)だが、プレイヤーが選ぶ明示的な方式である。

反対に、Electronic Artsは、同社がDDA技術の特許を持つ一方、FIFA、Madden、NHLのUltimate TeamではDDAや類似のスクリプトを使用していないと公式に説明している。[[7](#ref-7)] **特許の存在は、特定タイトルへの搭載証拠ではない。**

解析記事を使う場合は、対象プラットフォーム、バージョン、難易度、オンラインかオフラインか、再現手順を併記する。アップデートで挙動が変われば、過去の解析は現行版の証拠にならない。

---

## 7. DDAのリスクは「見つかること」だけではない
