...
```
# 仕上げAIへ図表制作依頼
優先度：必須。数値グラフではなく、器の分離を示す概念図を制作する。
図の対象は「第10版期のPDF運用」と図中に明記し、現行第11版のウェブページをPDFと誤認させないこと。
「書籍（有料・買い切り）」「ポイント値の文書（無料・更新される）」「公式アプリ（配信経路）」の3要素を置く。
書籍には「ルールの本文」、ポイント値の文書には「動く数値」と添える。
「ポイント値の文書」から「書籍の印刷済み数値」へ矢印を向け、「新しい数値が優先」と記す。上書きの方向を逆にしない。書籍のルール本文全体を上書きする図にはしない。
ポイント値の文書と公式アプリは「更新された数値を参照できる」と結び、内部のデータ送信処理を断定する矢印は描かない。
具体的なポイント数値、ユニット名、書籍名は入れない。推測で値や要素、配信処理を補わない。
```
...
```
# 仕上げAIへ図表制作依頼
優先度：推奨。更新サイクルの概念図を制作する。
対象は「第10版期に示された更新方針」と図中に明記する。
時間軸に沿って上段に「ポイント値の文書」の更新を四半期ごとに並べ、下段に「ルール変更の文書」の更新をその隔回に配置する。継続する関係が分かるよう、軸の両端は省略表現にする。
「目標であり、固定されたスケジュールではない」と図中に明記する。
具体的な年月、更新回数のラベル、次回予定日は入れない。実際の更新履歴を再現した図にしない。第11版の月次移行期間や現行の隔回更新を示す図として使わない。
推測で値や要素を追加しない。数値グラフとして扱わない。
```
