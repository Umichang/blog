...
敵AIの難しさは、最適な行動を計算することだけではない。こちらの位置を正確に把握して集中砲火する敵は、透視や後出しに見えやすい。反対に、味方につかえ、同じ壁へ走り続ける敵は故障に見える。

つまり必要なのは、単純な「頭の良さ」ではない。**プレイヤーが観察し、予測し、対処できる賢さ** である。ゲームAIの目的は知能そのものの再現ではなく、狙った体験を成立させることだという整理は、実務を考える出発点になる。[[1](#ref-1)]

本記事は、有限状態機械（FSM）、ビヘイビアツリー、GOAP、Utility AIを、歴史ではなく制作手段として比較する。各方式の歴史的な経緯は別記事「[コンピューターゲームにおけるAIの歴史](history-of-ai-in-video-games.md)」で扱っている。ここでは、何をデータにし、誰が調整し、どう壊れ方を見つけるかに集中する。
...
**FSM（Finite State Machine、有限状態機械）** は、「巡回中」「警戒中」「戦闘中」「逃走中」のような状態と、状態を切り替える遷移条件を定義する方式である。
...
**GOAP（Goal-Oriented Action Planning、目標指向型アクションプランニング）** は、達成したいゴールと現在の世界状態を比較し、利用可能なアクションをつないで計画を作る考え方である。
...
**Utility AI（効用ベースAI）** は、攻撃、退避、回復、接近などの候補行動へスコアを付け、現在もっとも望ましいものを選ぶ方式である。
...
| 敵の種類 | 雑魚、ボス、味方NPCで同じ基盤が必要か | 共通部品と固有ロジックの境界を決める |
...
| 性能 | 同時稼働数と対象機のCPU予算はどれくらいか | 探索・採点・感覚更新の頻度と対象数を制限する |
...
計画の違いが同じ走行と射撃にしか見えなければ、プレイヤーには伝わらない。専用モーション、台詞、レベル側の情報、QA項目まで含めて投資を見積もる必要がある。
...
これはグラフィックスのLODと同じく、AIの詳細度を重要度に応じて変える発想である。AI LODでは、距離だけでなく、プレイヤーから見た重要性と、簡略化が露見する危険を考えて処理資源を配る設計が提案されている。[[12](#ref-12)]
...
[5]: https://dev.epicgames.com/documentation/unreal-engine/behavior-tree-in-unreal-engine---overview
[6]: https://www.madwomb.com/tutorials/gamedesign/prototyping/gdc2006_JeffOrkin_AI_FEAR.pdf
