...
```
# 仕上げAIへ画像引用依頼
カプコン公式PR TIMES記事「祝！ 「大神」シリーズ20周年。記念特設サイトがオープン！」（2026年4月20日付、https://prtimes.jp/main/html/rd/p/000005743.000013450.html）に掲載されている20周年記念アート（デザイナー・島崎麻里氏の描き下ろし、アマテラスを中心に『大神』のキャラクターが並ぶイラスト）を引用候補としてください。同記事内の画像、またはカプコン公式20周年特設サイト（https://www.capcom-games.com/o-kami/20th/ja-jp/）に掲載されている同アートを想定しています。引用要件（必要な範囲・主従関係・出典明記・無改変）を満たせない場合はスキップしてください。
```
...
```
# 仕上げAIへ図表制作依頼
『大神』のハード展開を時系列で示す年表を作成してください。項目は次の6点。西暦年（月日が確認できるものは月日まで）とラベルのみを使い、売上や人気度などの評価的な数値は含めないでください。
・2006年4月20日：PlayStation 2版『大神』発売（クローバースタジオ開発）
・2009年10月15日：Wii版『大神』発売（Ready at Dawn Studios開発、操作系を再設計）
・2012年11月1日：PS3『大神 絶景版』パッケージ版発売（同年12月20日にダウンロード版配信）
・2017年：PS4/Xbox One/PC版『大神 絶景版』展開
・2024年12月13日：『大神 完全新作』プロジェクト発表（神谷英樹ディレクター）
・2026年4月20日：発売20周年
横方向の時系列バーまたはシンプルな年表形式で構いません。
```
...
```
# 仕上げAIへ画像引用依頼
PS2オリジナル版『大神』の水墨画風アートスタイルが分かるキービジュアルまたはパッケージ画像を引用候補としてください。カプコン公式サイト（https://www.capcom-games.com/o-kami/ など）や、PR TIMES 20周年記事内の「『大神』とは？」セクションに掲載されている画像（2006年発売時点のアートスタイルを示すもの）を想定しています。引用要件（必要な範囲・主従関係・出典明記・無改変）を満たせない場合はスキップしてください。
```
...
```
# 仕上げAIへ図表制作依頼
『大神』の開発初期における表現方針の転換を示すフロー図を作成してください。次の4段階を矢印でつなぐ簡潔な流れ図とし、各段階は本文の記述に基づく短いラベルのみを使ってください（数値データはありません）。
1. 当初の構想：自然を描く「明るい」方向のフォトリアル表現を志向（同じクローバースタジオ内の三上真司氏によるバイオハザードの「暗い」フォトリアルと対比）
2. 制約：PlayStation 2の描画能力では狙う画作りに届かず、試作が難航
3. 転機：デザイナーが日本画のタッチで描いたアマテラスの絵
4. 到達点：水墨画風のフィールド全体と「筆しらべ」システムの確立
```
...
```
# 仕上げAIへ画像引用依頼
Wii版『大神』（2009年）とPS3『大神 絶景版』（2012年）の操作画面またはUIスクリーンショットを、可能であれば並べて引用候補としてください。特にWiiリモコンでのポインティング操作中の画面（GAME Watchのファーストインプレッション記事等）と、PS3版の通常コントローラー／PlayStation Move操作の画面を想定しています。両方の要件を満たす画像が揃わない場合は、片方のみ、または本文の趣旨に沿わなければスキップしてください。
```
...
```
# 仕上げAIへ図表制作依頼
『大神』の「筆しらべ」操作方式を、3つのハードで対比するフロー図または比較図を作成してください。軸は「入力装置」と「操作の位置づけ」の2点とし、以下の内容を使ってください（数値データはありません）。
・PlayStation 2版（2006年）：DUALSHOCK 2のアナログスティックで筆のカーソルを動かし、ボタンで線を引く
・Wii版（2009年）：Wiiリモコン＋ヌンチャク専用。Wiiリモコンのポインターを筆先として動かし、Aボタンを押しながら線を描く。入力方式そのものを再設計
・PS3『大神 絶景版』（2012年）：通常のワイヤレスコントローラーを基準として維持しつつ、PlayStation Moveによる操作を選択肢として追加
「PS2→Wii→PS3絶景版」の順で、それぞれの操作方式の違いが一目でわかる構成にしてください。
```
...
```
# 仕上げAIへ図表制作依頼
移植時の操作設計における2つの戦略を対比する概念図を作成してください。本文の表（「入力を作り直す」「維持しつつ選択肢を足す」）を視覚化したもので、数値データは使いません。
・入力を作り直す：新しい入力が中心行為と強く対応し、全員がその入力を使える場合に選ぶ。行為の意味と身体動作を近づけられる。UI・チュートリアル・判定も一体で調整する必要がある（例：Wii版）
・維持しつつ選択肢を足す：既存操作が成立しており、技術更新や到達範囲の拡張が主目的の場合に選ぶ。原作の手触りを保ち、異なる好みにも入口を作れる。二つの操作経路が一方的に不利にならないよう検証する必要がある（例：PS3『大神 絶景版』）
二択のマトリクスまたは対比イラストなど、プランナーが実務で参照しやすい形式にしてください。
```
...
2024年12月には、神谷をディレクターに迎える『大神 完全新作』プロジェクトが発表された。CLOVERS、Machine Head Works、M2の共同開発である[[4](#ref-4)]。続編がどの入力装置を選ぶかはまだ公表されていない。それでも過去の二つの再解釈は、次の企画で問うべき基準を残している。新しいハードの機能を採るかどうかではなく、その機能によってプレイヤーが作品の中で行う因果――線を引き、世界に働きかける感覚――を、より明瞭にできるかである。
...
<a id="ref-4"></a>4. [この世の命が、蘇る。『大神 完全新作』プロジェクトが始動！][4] - 神谷英樹が2006年『大神』のディレクションを担ったこと、2024年の正式続編プロジェクト発表、続編での神谷英樹のディレクター就任、CLOVERS・Machine Head Works・M2による共同開発を確認した。
