...
トレーディングカードゲーム、以下TCGは、カードを集め、選び、組み合わせ、対戦で検証する遊びである。紙のTCGでは、物理カードを買い、交換し、デッキケースに入れ、対面でシャッフルする。この制約は単なる不便ではない。デッキ枚数、同名カード上限、サイドボード、レアリティ、ローテーションといった仕組みは、物理的な商品と大会運営の制約から生まれ、結果として戦略の骨格になった。

```
# 仕上げAIへ画像引用依頼
紙のTCGの対戦・デッキ構築風景の写真を挿入してほしい。物理カード、デッキケース、シャッフルしている様子など、紙TCGの手触りが伝わるアングルが望ましい。特定タイトルのブランドに依存しない一般的な写真が望ましく、著作権法上の引用要件(必要な範囲・主従関係・出典明記・無改変)を満たす画像、またはフリーライセンス画像を選定し、出典を明記すること。要件を満たす画像が見つからない場合はその旨を明記してスキップすること。
```
...
```markdown
# 仕上げAIへ図表制作依頼
紙TCGとデジタルTCGの違いを比較する横長のインフォグラフィックを作成してください。
左に「紙TCG」、右に「デジタルTCG」を配置し、中央に「引き継がれる設計思想」を置く構成。
紙側には「物理カード」「対面確認」「印刷後は変更困難」「大会運用で補正」。
デジタル側には「自動裁定」「秘匿情報をサーバー管理」「カード生成」「オンライン調整」。
中央には「デッキ枚数」「同名上限」「カードプール」「メタゲーム」「収集と構築」を短く置く。
文字量は少なめにし、ゲームプランナー向けの落ち着いた資料風にしてください。
```
...
```
# 仕上げAIへ図表制作依頼
メタゲームの循環構造を示す円環図を作成してください。
「あるデッキが流行する」→「対策デッキが増える」→「対策デッキへの対策が現れる」→(最初に戻る)という3〜4ステップの循環を矢印でつなぐ。
円環の外側に「新弾投入」「ローテーション」「禁止・制限」「エラッタ・数値調整」という運営側の介入手段を短く添える。
ゲームプランナー向けの落ち着いた資料風にし、文字量は少なめにしてください。
```
...
```markdown
# 仕上げAIへ図表制作依頼
デッキ構築支援UIの画面構成を、ワイヤーフレーム風の図で作成してください。
左にカード一覧と検索フィルタ、中央にデッキリスト、右にコスト曲線と不足カード表示。
下部に「初心者テンプレート」「代替カード提案」「フォーマット合法性チェック」の小さなパネルを置く。
実在タイトルの画面模写ではなく、汎用的な企画資料として読める構成にしてください。
文字は短く、カード名は架空名にしてください。
```

```
# 仕上げAIへ画像引用依頼
実在するデジタルTCGのデッキ構築UI画面のスクリーンショットを1点挿入してほしい。カード一覧、検索フィルタ、コスト曲線表示などが分かるアングルが望ましい。直前のワイヤーフレーム図(概念)と対になる実例として使う。著作権法上の引用要件(必要な範囲・主従関係・出典明記・無改変)を満たす画像を選定し、出典を明記すること。要件を満たす画像が見つからない場合はその旨を明記してスキップすること。
```
...
```
# 仕上げAIへ画像引用依頼
MARVEL SNAP（マーベル・スナップ）のゲーム画面のスクリーンショットを1点挿入してほしい。12枚デッキ、短時間マッチという特徴が伝わるアングルが望ましい。著作権法上の引用要件(必要な範囲・主従関係・出典明記・無改変)を満たす画像を選定し、出典を明記すること。要件を満たす画像が見つからない場合はその旨を明記してスキップすること。
```
