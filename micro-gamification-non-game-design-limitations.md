...
## 図表：小さな仕掛けを「体験」に変える設計図

- 挿入位置：このエグゼクティブサマリーの直後
- 目的：ポイント、ストリークなどの部品と、利用者に生む体験を混同しないという本稿の主張を一目で示す
- 形式：横長、16:9、日本語のフラットなインフォグラフィック。白またはごく薄いグレーの背景、濃紺の文字、青緑をアクセントにする。企業ロゴ、人物、ゲームキャラクター、装飾的な3D表現は入れない
- 構成：左から右へ3列の矢印図
  - 左列の見出し「小さな仕掛け」：マイクロチャレンジ／進捗表示／ストリーク／バッジ／リーダーボード
  - 中列の見出し「利用者に返す体験」：次の一歩が分かる／上達が読める／休んでも戻れる／できたことに名前が付く／比較の意味を選べる
  - 右列の見出し「設計の目的」：自律性／有能感／関係性
- 画面下部に短い結論として「部品を足すだけでは、体験は生まれない」を配置する
- 文字は上記の語句だけに絞り、各項目は読みやすい大きさにする
```
...
## DuolingoのStreak Freeze画面

- 挿入位置：直前の、DuolingoのStreak FreezeとWeekend Amuletを説明する段落の直後
- 引用元：https://blog.duolingo.com/protecting-streaks-from-site-issues/
- 取得対象：記事中の「A repaired streak using streak freeze」と説明された、欠席日の記録をStreak Freezeで保護する画面
- 引用目的：連続記録を厳しく維持するだけでなく、中断後の復帰を支える仕組みが必要だという本文の具体例として示す
- 表示：画面の内容が読める最小限の大きさで引用し、WebPへ変換する。キャプションに「画像出典（引用）：Duolingo Blog『How we protect learner streaks from site issues』／Streak Freezeによる連続記録の保護例として引用。WebP変換。」と明記する
```

習慣形成に「21日」という万能の期限はない。日常行動を追った研究では、自動化の進み方には大きな個人差があり、推定値は18日から254日に及んだ。また、一回の取りこぼしは習慣形成の過程を大きく損なわなかった。ストリークを設計するなら、連続日数を神聖化するより、戻る行為を成功として数えるべきである。[[6](#ref-6)]
...
- 挿入位置：この「ストリークは『失わせない設計』まで含めて完成する」節の末尾
- 目的：ストリークを善悪で断じるのではなく、中断後の復帰導線が体験を分けることを示す
- 形式：横長、16:9、日本語のフラットなフローチャート。白背景。左側は控えめな赤、右側は青緑を使う。人物、ブランドロゴ、ゲームキャラクターは入れない
- 構成：左右対照の2本のフロー
  - 左の見出し「ゼロリセットだけのループ」：連続記録 → 1日中断 → 記録がゼロ → 最小操作だけをする／離脱
  - 右の見出し「復帰できるループ」：連続記録 → 1日中断 → 休止・救済 → 再開 → 継続
- 下部に「ストリークの価値は、失わせないことではなく、戻れることにある」と大きく配置する
- 左右の違いが一目で分かるよう、矢印と余白を十分に取る。本文を写した細かい注釈は入れない
```
...
ただしAIが最適化しやすいのは、クリック数、滞在時間、連続日数のような測りやすい数字である。その数字が利用者の利益とずれていれば、個別化はより精密な圧力になる。教育での生成AI利用について、UNESCOもデータプライバシーの保護と人間中心の統治を求めている。[[7](#ref-7)]
...
<a id="ref-6"></a>6. [How are habits formed: Modelling habit formation in the real world][6] - 日常行動の反復と自動化を追跡し、習慣形成の時間に大きな個人差があることを示した研究。

<a id="ref-7"></a>7. [Guidance for generative AI in education and research][7] - 教育・研究における生成AI利用について、データプライバシーを含む統治上の論点をまとめたUNESCOの指針。
...
[6]: https://www.flexyourbrain.com/wp-content/uploads/2015/10/UL-STUDY-66-Days-IJSP_998-1009.pdf
[7]: https://www.unesco.org/en/articles/guidance-generative-ai-education-and-research?hub=343
