...
```
# 仕上げAIへ図表制作依頼
数値グラフではない概念図。中央に「テキストだけで伝えるキャラクターの声」という見出しを置き、左に Recognizability（ひと目で誰か分かる）を表す虫眼鏡とキャラクターの輪郭、右に Authenticity（その人物が実在しそうに感じる）を表す会話吹き出しと自然な文章の輪郭を配置する。中央から左右へ「綴り・語彙・リズム・周囲の反応」の4要素が橋渡しする構成。日本語ラベルを大きく少なくし、ゲーム開発の技術記事に合う落ち着いた青緑と橙の配色にする。画像内に Fig. 番号は入れない。
```
...
```
# 仕上げAIへ画像引用依頼
GAME Watch記事「Umazing！『ウマ娘』英語版では方言や口調をいかに再現したか」（https://game.watch.impress.co.jp/docs/news/2127676.html）内の、ダイタクヘリオスのZ世代スラング表現を解説したスライド画像（記事中の画像ファイル名はcedec_uma_10、キャプション「『ギャル』という概念が海外では伝わりにくいため、Z世代(GenZ)の言葉を適用させたという」）を引用候補とする。著作権法上の引用要件（必要な範囲・主従関係・出典明記・無改変）を満たす形で、本文の該当箇所への挿入を検討してほしい。出典表記は「出典：GAME Watch（Impress Corporation）」とする。要件を満たせないと判断した場合はスキップしてよい。
```
...
```
# 仕上げAIへ図表制作依頼
4体のウマ娘（タマモクロス、ユキノビジン、アグネスデジタル、ダイタクヘリオス）を横並びに比較する概念図。各キャラクターについて「方言・口調の特徴」「翻訳上の課題」「採用した技法」の3段を縦に並べた列として構成する。タマモクロス列＝関西弁／南部訛りとの衝突を避け英語の綴りでリズムを再現、ユキノビジン列＝岩手弁／地域方言ではなく1930〜60年代の古い英語表現を採用、アグネスデジタル列＝オタク口調／英語圏オタクコミュニティの実際の言い回しをリサーチ、ダイタクヘリオス列＝ギャル口調／Z世代スラングへ再定義。4列を並べたときに「地域方言への直訳」という定石から外れ、それぞれ別の軸（音・時代・コミュニティ・世代）で解決している点が一目でわかるようにする。数値は使わない概念図。日本語ラベルのみで、ゲーム開発の技術記事に合う落ち着いた配色にする。Fig.番号は入れない。
```
...
```
# 仕上げAIへ図表制作依頼
口調ガイドラインが完成するまでの流れを示すシンプルな工程図（左から右、または上から下への4ステップ）。①キャラクターや関連文化に詳しいメンバーが最初の下訳を担当→②その担当者が注意点をまとめた口調ガイドラインのドラフトを作成→③言語品質管理チームが校正・確認→④担当者が変わっても一貫性を保てるガイドラインとして完成、の4段階を矢印でつなぐ。各ステップは短い日本語ラベルのみとし、人物アイコンや吹き出しなど装飾は最小限に留める。数値グラフではない概念図。Fig.番号は入れない。
```
...
なお、英語版『ウマ娘 プリティーダービー』はThe Game Awards 2025のBest Mobile GameでWinnerとして掲載されている。[[4](#ref-4)] ただし、この賞を特定の口調ローカライズだけの成果とみなすことはできない。ここで確認できるのは、作品全体が海外で受け入れられた事実の一端であり、翻訳表現の因果を単独で証明するものではない。
