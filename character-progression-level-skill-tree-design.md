...
```
# 仕上げAIへ図表制作依頼
主題:経験値カーブ比較(線形・指数・対数)の折れ線グラフ
構成イメージ:横軸をレベル(Lv.1〜11)、縦軸を必要XPとした折れ線グラフ。線形(a=100、b=50)、指数(a=100、r=1.28)、対数(a=100、b=220)の3本を色分けして重ねる。本文の数値表と対応させる。
ファイル名:xp-curve-comparison-linear-exponential-logarithmic.webp
alt候補:線形・指数・対数の3つの経験値カーブを比較する折れ線グラフ
キャプション文言案:同じLv.1→2の必要XPから出発しても、カーブ型によって後半の伸び方は大きく異なる
```
...
# 仕上げAIへ図表制作依頼
主題:ソフトキャップ逓減曲線のグラフ
構成イメージ:横軸を投入値x、縦軸を効果B(x)とした曲線グラフ。B_maxを漸近線として描き、Kの位置(効果が上限の半分に達する点)を目印として示す。
ファイル名:softcap-diminishing-returns-curve.webp
alt候補:投入量に対して効果が頭打ちになるソフトキャップの逓減曲線
キャプション文言案:投入量が増えるほど、1ポイントの効果は小さくなる
```
...
```
# 仕上げAIへ図表制作依頼
主題:スキルツリー構造3類型(直線型・分岐型・グリッド型)の比較図
構成イメージ:3つのパネルを横並びにし、それぞれノードと接続線の模式図を示す。直線型は縦一本、分岐型は幹から複数の枝、グリッド型は格子状の網として描く。各パネル下に強み・主な事故を1行ずつ添える。
ファイル名:skill-tree-structure-three-types-comparison.webp
alt候補:直線型・分岐型・グリッド型の3つのスキルツリー構造を比較する図
キャプション文言案:構造の型は、選択の見せ方と迷いやすさのトレードオフを決める
```
...
# 仕上げAIへ図表制作依頼
主題:育成データモデルの関係図
構成イメージ:アカウント/キャラクター/サーバーの3つのスコープを枠で分け、表7の主要データ項目(level、total_xp、allocated_skill_node_ids、respec_history等)をそれぞれ対応する枠内に配置する関係図。
ファイル名:progression-data-model-scope-diagram.webp
alt候補:育成データ項目をアカウント・キャラクター・サーバーのスコープ別に整理した関係図
キャプション文言案:表の列を先に決めるとは、データの所有スコープを先に決めることでもある
```
...
ここで扱うのは収益戦略全体ではなく、育成アイテムが競争条件や選択を壊さないための判定である。「ペイ・トゥ・ウィン」に単一の公的定義があると仮定せず、次の五問で評価する。
...
```
# 仕上げAIへ図表制作依頼
主題:育成とマネタイズの境界、判定フロー図
構成イメージ:本文の5つの問いを上から順に並べたディシジョンツリー。各問いに対してYes/Noの分岐を作り、「比較的リスクを抑えやすい」側と「高リスクになる」側へ収束させる。
ファイル名:progression-monetization-boundary-decision-flow.webp
alt候補:育成アイテムがペイ・トゥ・ウィンかどうかを5つの問いで判定するフロー図
キャプション文言案:単一の定義に頼らず、5つの問いの積み重ねでリスクを判定する
```
...
```
# 仕上げAIへ図表制作依頼
主題:育成UIプレビュー画面のワイヤーフレーム
構成イメージ:スキルノード選択画面を想定したワイヤーフレーム。消費ポイントと残ポイント、解放される能動スキル、閉じる排他枝、ステータスの変更前→変更後の差分表示を1画面レイアウトとして図解する。
ファイル名:progression-ui-preview-wireframe.webp
alt候補:育成UIの確定前プレビュー画面のワイヤーフレーム
キャプション文言案:確定前に、消費・差分・閉じる枝がまとめて見えることが最低条件になる
```
...
# 仕上げAIへ図表制作依頼
主題:XPカーブ移行の概念図
構成イメージ:旧カーブと新カーブを並べた2本の折れ線として描き、旧カーブ上のあるレベルの進捗率(q)から、新カーブ上の対応する位置へ矢印でつなぐ。同じ進捗率が新カーブでは異なるXP値に対応することを視覚化する。
ファイル名:xp-curve-migration-progress-ratio-mapping.webp
alt候補:旧カーブの進捗率を新カーブへ変換する仕組みを示す図
キャプション文言案:レベルとレベル内進捗率を保ったまま、カーブだけを差し替える
```
